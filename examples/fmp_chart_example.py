...
async def historical_price_examples(client: FmpClient) -> None:
    ...
    light_data = await client.chart.historical_price_light("AAPL", "2025-01-01", "2025-01-31")
    ...
    full_data = await client.chart.historical_price_full("AAPL", "2025-01-01", "2025-01-31")
    ...
    unadjusted_data = await client.chart.historical_price_unadjusted("AAPL", "2025-01-01", "2025-01-31")
    ...
    dividend_adjusted_data = await client.chart.historical_price_dividend_adjusted("AAPL", "2025-01-01", "2025-01-31")
...
async def chart_analysis_examples(client: FmpClient) -> None:
    ...
    try:
        ...
        light_data = await client.chart.historical_price_light("AAPL", from_date_month, to_date)
        print(f"  Historical Price (Light): {len(light_data)} data points")
        
        # Historical price data (full)
        full_data = await client.chart.historical_price_full("AAPL", from_date_month, to_date)
        print(f"  Historical Price (Full): {len(full_data)} data points")
        
        # Intraday data (1-hour)
        intraday_data = await client.chart.intraday_1hour("AAPL", from_date_week, to_date)
        ...
        if light_data:
            volumes = [data.get('volume', 0) for data in light_data if data.get('volume')]
            if volumes:
        ...
        if full_data and light_data:
            ...
            if len(full_data) > 0 and len(light_data) > 0:
                ...
                print(f"  Sample Full Data: OHLC + Volume + Changes + VWAP")
                print(f"  Sample Light Data: Price + Volume only")
...
async def multi_symbol_chart_example(client: FmpClient) -> None:
    ...
    for symbol in symbols:
        ...
        try:
            ...
            light_data = await client.chart.historical_price_light(symbol, "2025-01-01", "2025-01-31")
            if light_data:
            ...
            # Get intraday data
            intraday_data = await client.chart.intraday_1hour(symbol, "2025-01-01", "2025-01-02")
            if intraday_data:
                print(f"  Intraday Data Points: {len(intraday_data)}")
                
                # Show latest intraday data
                latest_intraday = intraday_data[0]
                print(f"  Latest Intraday Close: ${latest_intraday.get('close', 0):.2f}")
...
async def nonadjusted_data_example(client: FmpClient) -> None:
    ...
    nonadjusted_data = await client.chart.intraday_1min("AAPL", "2025-01-01", "2025-01-02", nonadjusted=True)
    ...
    adjusted_data = await client.chart.intraday_1min("AAPL", "2025-01-01", "2025-01-02", nonadjusted=False)
    ...
    if nonadjusted_data and adjusted_data:
        ...
        if len(nonadjusted_data) > 0 and len(adjusted_data) > 0:
            ...
            print(f"  Sample Nonadjusted: {nonadj_sample.get('date', 'N/A')} - Close: ${nonadj_sample.get('close', 0):.2f}")
            print(f"  Sample Adjusted: {adj_sample.get('date', 'N/A')} - Close: ${adj_sample.get('close', 0):.2f}")
