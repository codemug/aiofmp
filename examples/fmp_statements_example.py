...
async def income_statement_examples(client: FmpClient) -> None:
    ...
    income_stmt = await client.statements.income_statement("AAPL", limit=5, period="annual")
    ...
    msft_income = await client.statements.income_statement("MSFT", limit=3, period="annual")
...
async def balance_sheet_examples(client: FmpClient) -> None:
    ...
    balance_sheet = await client.statements.balance_sheet_statement("AAPL", limit=5, period="annual")
    ...
    if balance_sheet:
        ...
        for i, record in enumerate(balance_sheet):
            ...
            print(f"     Total Stockholders' Equity: ${record.get('totalStockholdersEquity', 0):,}")
            print(f"     Cash & Cash Equivalents: ${record.get('cashAndCashEquivalents', 0):,}")
...
async def cash_flow_examples(client: FmpClient) -> None:
    ...
    cash_flow = await client.statements.cash_flow_statement("AAPL", limit=5, period="annual")
    ...
    if key_metrics:
        ...
        for i, record in enumerate(key_metrics):
            ...
            if market_cap > 0 and enterprise_value > 0:
                ...
                print(f"     Enterprise Value to Market Cap Ratio: {ev_to_mc_ratio:.3f}")
...
async def financial_ratios_examples(client: FmpClient) -> None:
    ...
    financial_ratios = await client.statements.financial_ratios("AAPL", limit=5, period="annual")
    ...
    if financial_ratios:
        ...
        for i, record in enumerate(financial_ratios):
            ...
            print(f"       Gross Profit Margin: {record.get('grossProfitMargin', 0):.3f}")
            ...
            print(f"       Inventory Turnover: {record.get('inventoryTurnover', 0):.3f}")
            print(f"       Receivables Turnover: {record.get('receivablesTurnover', 0):.3f}")
            ...
            print(f"       Financial Leverage: {record.get('financialLeverageRatio', 0):.3f}")
    ...
    if financial_scores:
        ...
        for record in financial_scores:
            ...
            if piotroski_score >= 7:
                print(f"  🟢 Piotroski Score: {piotroski_score}/9 - Strong Financial Health (≥7)")
            elif piotroski_score >= 4:
                print(f"  🟡 Piotroski Score: {piotroski_score}/9 - Moderate Financial Health (4-6)")
            else:
                print(f"  🔴 Piotroski Score: {piotroski_score}/9 - Weak Financial Health (<4)")
...
async def owner_earnings_examples(client: FmpClient) -> None:
    ...
    if owner_earnings:
        ...
        for i, record in enumerate(owner_earnings):
            ...
            print(f"     Owner Earnings Per Share: ${record.get('ownersEarningsPerShare', 0):.2f}")
...
async def enterprise_values_examples(client: FmpClient) -> None:
    ...
    enterprise_values = await client.statements.enterprise_values("AAPL", limit=5, period="annual")
    ...
    if enterprise_values:
        ...
        for i, record in enumerate(enterprise_values):
            ...
            print(f"     Market Capitalization: ${record.get('marketCapitalization', 0):,}")
            print(f"     Minus Cash & Cash Equivalents: ${record.get('minusCashAndCashEquivalents', 0):,}")
            ...
            if stock_price > 0:
                ...
                if shares_outstanding > 0:
                    ...
                    print(f"     Enterprise Value Per Share: ${book_value_per_share:.2f}")
...
async def growth_analysis_examples(client: FmpClient) -> None:
    ...
    income_growth = await client.statements.income_statement_growth("AAPL", limit=5, period="annual")
    ...
    if income_growth:
        ...
        for i, record in enumerate(income_growth):
            ...
            print(f"     Operating Income Growth: {record.get('growthOperatingIncome', 0):.2%}")
            ...
            if revenue_growth > 0 and net_income_growth > 0:
                print(f"     ✅ Both revenue and net income growing")
            elif revenue_growth > 0 and net_income_growth < 0:
                print(f"     ⚠️  Revenue growing but net income declining")
            elif revenue_growth < 0 and net_income_growth < 0:
                print(f"     🔴 Both revenue and net income declining")
            else:
                print(f"     🔄 Mixed growth patterns")
    
    # Get comprehensive financial growth for Apple
    print("\nFetching comprehensive financial growth for Apple (AAPL)...")
    financial_growth = await client.statements.financial_statement_growth("AAPL", limit=3, period="annual")
    ...
    if financial_growth:
        ...
        for i, record in enumerate(financial_growth):
            ...
            print(f"     Operating Cash Flow Growth: {record.get('operatingCashFlowGrowth', 0):.2%}")
            print(f"     Free Cash Flow Growth: {record.get('freeCashFlowGrowth', 0):.2%}")
            print(f"     Asset Growth: {record.get('assetGrowth', 0):.2%}")
            print(f"     Debt Growth: {record.get('debtGrowth', 0):.2%}")
            
            # Long-term growth trends
            print(f"     Long-term Growth Trends:")
            print(f"       10Y Revenue Growth Per Share: {record.get('tenYRevenueGrowthPerShare', 0):.2%}")
            print(f"       5Y Revenue Growth Per Share: {record.get('fiveYRevenueGrowthPerShare', 0):.2%}")
            print(f"       3Y Revenue Growth Per Share: {record.get('threeYRevenueGrowthPerShare', 0):.2%}")
...
async def revenue_segmentation_examples(client: FmpClient) -> None:
    ...
    product_segmentation = await client.statements.revenue_product_segmentation("AAPL", period="annual")
    ...
    geographic_segmentation = await client.statements.revenue_geographic_segmentation("AAPL", period="annual")
    print(f"Found {len(geographic_segmentation)} geographic segmentation records for AAPL")
...
async def financial_analysis_examples(client: FmpClient) -> None:
    ...
    income_stmt = await client.statements.income_statement("AAPL", limit=1, period="annual")
    balance_sheet = await client.statements.balance_sheet_statement("AAPL", limit=1, period="annual")
    cash_flow = await client.statements.cash_flow_statement("AAPL", limit=1, period="annual")
    key_metrics = await client.statements.key_metrics("AAPL", limit=1, period="annual")
    financial_ratios = await client.statements.financial_ratios("AAPL", limit=1, period="annual")
    financial_scores = await client.statements.financial_scores("AAPL")
    
    if income_stmt and balance_sheet and cash_flow and key_metrics and financial_ratios and financial_scores:
        ...
        print(f"  Gross Margin: {(gross_profit/revenue)*100:.1f}%" if revenue > 0 else "  Gross Margin: N/A")
        print(f"  Net Margin: {(net_income/revenue)*100:.1f}%" if revenue > 0 else "  Net Margin: N/A")
        ...
        if total_assets > 0 and total_liabilities > 0:
            ...
            if debt_ratio < 0.3:
                print(f"  ✅ Strong financial position (Low debt ratio: {debt_ratio:.3f})")
            elif debt_ratio < 0.5:
                print(f"  🟡 Moderate financial position (Debt ratio: {debt_ratio:.3f})")
            else:
                print(f"  ⚠️  High leverage (Debt ratio: {debt_ratio:.3f})")
        
        # Cash flow assessment
        if operating_cf > 0 and free_cf > 0:
            print(f"  ✅ Strong cash generation (Operating CF: ${operating_cf:,}, Free CF: ${free_cf:,})")
        elif operating_cf > 0:
        ...
        if revenue > 0 and net_income > 0:
            ...
            print(f"  💡 Strong fundamentals with solid cash flow generation")
