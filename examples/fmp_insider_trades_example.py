...
async def latest_insider_trades_examples(client: FmpClient) -> None:
    ...
    if latest_trades:
        ...
        acquisitions = [t for t in latest_trades if t.get('acquisitionOrDisposition') == 'A']
        dispositions = [t for t in latest_trades if t.get('acquisitionOrDisposition') == 'D']
...
async def search_insider_trades_examples(client: FmpClient) -> None:
    ...
    if apple_trades:
        ...
        for trade in apple_trades:
            trans_type = trade.get('transactionType', 'Unknown')
            apple_transaction_types[trans_type] = apple_transaction_types.get(trans_type, 0) + 1
        
        print(f"\n📊 Apple Insider Trading by Type:")
        for trans_type, count in sorted(apple_transaction_types.items(), key=lambda x: x[1], reverse=True):
            print(f"  {trans_type}: {count} trades")
        ...
        if apple_sales:
            ...
            for i, sale in enumerate(apple_sales[:5]):
                ...
                print(f"    {i+1}. {name}: {shares:,} shares at ${price:.2f}" if price > 0 else f"    {i+1}. {name}: {shares:,} shares")
    ...
    if msft_trades:
        ...
        for trade in msft_trades:
            trans_type = trade.get('transactionType', 'Unknown')
            msft_transaction_types[trans_type] = msft_transaction_types.get(trans_type, 0) + 1
        
        for trans_type, count in sorted(msft_transaction_types.items(), key=lambda x: x[1], reverse=True):
            print(f"    {trans_type}: {count} trades")
                ...
                if results:
                    ...
                    if trades:
                        ...
                        for i, trade in enumerate(trades[:5]):
                            ...
                            print(f"      {i+1}. {symbol}: {transaction_type} on {filing_date} ({securities:,} shares)")
                    else:
        ...
        try:
            ...
            if stats:
                for stat in stats:
                    ...
                    print(f"    Transactions: {acquired_transactions} acquired, {disposed_transactions} disposed")
                    print(f"    Ratio: {acquired_disposed_ratio:.4f}")
                    print(f"    Total: {total_acquired:,} acquired, {total_disposed:,} disposed")
                    print(f"    Average: {average_acquired:,.0f} acquired, {average_disposed:,.0f} disposed")
                    print(f"    Purchases: {total_purchases}, Sales: {total_sales}")
                    
                    # Sentiment analysis
                    if acquired_disposed_ratio > 1:
                        print(f"    🟢 Bullish sentiment (more acquisitions than dispositions)")
                    elif acquired_disposed_ratio < 1:
                        print(f"    🔴 Bearish sentiment (more dispositions than acquisitions)")
                    else:
                        print(f"    ➡️  Neutral sentiment (equal acquisitions and dispositions)")
                    
                    # Activity level
                    total_transactions = acquired_transactions + disposed_transactions
                    if total_transactions > 20:
                        print(f"    📈 High insider activity")
                    elif total_transactions > 10:
                        print(f"    📊 Moderate insider activity")
                    else:
                        print(f"    📉 Low insider activity")
            else:
...
async def acquisition_ownership_examples(client: FmpClient) -> None:
    ...
    for symbol in companies:
        ...
        try:
            ownership_changes = await client.insider_trades.acquisition_ownership(symbol, limit=100)
            print(f"  Found {len(ownership_changes)} ownership changes")
            
            if ownership_changes:
                print(f"  Recent Ownership Changes:")
                ...
                for person_type, count in sorted(person_types.items(), key=lambda x: x[1], reverse=True):
                    print(f"    {person_type}: {count} changes")
                
                # Find significant ownership changes (>5%)
                significant_changes = [
                    change for change in ownership_changes
...
async def insider_analysis_examples(client: FmpClient) -> None:
    ...
    for symbol in major_companies:
        ...
        try:
            # Get insider trades
            trades = await client.insider_trades.search_insider_trades(symbol, limit=100)
            
            if trades:
                print(f"  📈 Insider Trading Summary:")
                ...
                # Analyze by acquisition/disposition
                acquisitions = [t for t in trades if t.get('acquisitionOrDisposition') == 'A']
                dispositions = [t for t in trades if t.get('acquisitionOrDisposition') == 'D']
                
                print(f"    Acquisitions: {len(acquisitions)}, Dispositions: {len(dispositions)}")
                
                # Sentiment analysis
                if len(acquisitions) > len(dispositions):
                    print(f"    🟢 Bullish insider sentiment")
                elif len(dispositions) > len(acquisitions):
                    print(f"    🔴 Bearish insider sentiment")
                else:
                    print(f"    ➡️  Neutral insider sentiment")
                ...
                if stats:
                    ...
                    if ratio > 1:
                        print(f"      📈 More acquisitions than dispositions")
                    elif ratio < 1:
                        print(f"      📉 More dispositions than acquisitions")
                    else:
                        print(f"      ➡️  Equal acquisitions and dispositions")
                
                # Recent activity
                recent_trades = [t for t in trades if t.get('filingDate', '') >= '2024-01-01']
    ...
    try:
        ...
        latest_trades = await client.insider_trades.latest_insider_trades(page=0, limit=200)
        ...
        if latest_trades:
            ...
            print(f"  🟢 Acquisitions: {len(acquisitions)} ({len(acquisitions)/total_trades*100:.1f}%)")
            print(f"  🔴 Dispositions: {len(dispositions)} ({len(dispositions)/total_trades*100:.1f}%)")
            
            # Overall sentiment
            if len(acquisitions) > len(dispositions):
                print(f"  📈 Overall Market Sentiment: Bullish")
            elif len(dispositions) > len(acquisitions):
                print(f"  📉 Overall Market Sentiment: Bearish")
            else:
                print(f"  ➡️  Overall Market Sentiment: Neutral")
            ...
            top_companies = sorted(company_activity.items(), key=lambda x: x[1], reverse=True)[:10]
            for symbol, count in top_companies:
                print(f"    {symbol}: {count} trades")
