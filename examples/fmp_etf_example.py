...
async def holdings_examples(client: FmpClient) -> None:
    ...
    if spy_holdings:
        ...
        for i, holding in enumerate(spy_holdings[:5]):  # Show first 5 holdings
            print(f"  {i+1}. {holding.get('name', 'N/A')} ({holding.get('asset', 'N/A')})")
...
async def info_examples(client: FmpClient) -> None:
    ...
    if spy_info:
        ...
        print(f"  Assets Under Management: ${info.get('assetsUnderManagement', 0):,.0f}")
...
async def country_weightings_examples(client: FmpClient) -> None:
    ...
    if spy_countries:
        ...
        for i, country in enumerate(spy_countries):
            print(f"  {i+1}. {country.get('country', 'N/A')}: {country.get('weightPercentage', 'N/A')}")
        print()
...
async def sector_weightings_examples(client: FmpClient) -> None:
    ...
    if spy_sectors:
        ...
        for i, sector in enumerate(spy_sectors):
            print(f"  {i+1}. {sector.get('sector', 'N/A')}: {sector.get('weightPercentage', 0):.2f}%")
        print()
...
async def etf_analysis_examples(client: FmpClient) -> None:
    ...
    if spy_sectors and qqq_sectors:
        ...
        spy_top_sectors = sorted(spy_sectors, key=lambda x: x.get('weightPercentage', 0), reverse=True)[:3]
        qqq_top_sectors = sorted(qqq_sectors, key=lambda x: x.get('weightPercentage', 0), reverse=True)[:3]
        ...
        if spy_concentration > 50 or qqq_concentration > 50:
            print("    Note: High sector concentration (>50%) indicates concentrated risk")
        else:
            print("    Note: Moderate sector concentration indicates diversified exposure")
...
async def etf_portfolio_analysis(client: FmpClient) -> None:
    ...
    top_countries = sorted(country_exposure.items(), key=lambda x: x[1], reverse=True)[:5]
    print(f"\nTop Country Exposures:")
