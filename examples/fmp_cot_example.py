...
async def cot_report_examples(client: FmpClient) -> None:
    ...
    if cot_report:
        ...
        for i, record in enumerate(cot_report[:3]):
            print(f"  {i+1}. {record.get('name', 'N/A')} ({record.get('symbol', 'N/A')})")
            ...
            print(f"     Non-Commercial Long: {record.get('noncommPositionsLongAll', 0):,}")
            print(f"     Non-Commercial Short: {record.get('noncommPositionsShortAll', 0):,}")
            print(f"     Commercial Long: {record.get('commPositionsLongAll', 0):,}")
            print(f"     Commercial Short: {record.get('commPositionsShortAll', 0):,}")
            print(f"     Total Reportable Long: {record.get('totReptPositionsLongAll', 0):,}")
            print(f"     Total Reportable Short: {record.get('totReptPositionsShortAll', 0):,}")
            print()
    ...
    if ng_cot:
        ...
        for i, record in enumerate(ng_cot[:2]):
            ...
            print(f"     Non-Commercial Positions:")
...
async def cot_analysis_examples(client: FmpClient) -> None:
    ...
    if cot_analysis:
        ...
        for i, record in enumerate(cot_analysis[:3]):
            ...
            print(f"     Current Market Situation: {record.get('marketSituation', 'N/A')}")
            print(f"     Current Long: {record.get('currentLongMarketSituation', 0):.2f}%")
            print(f"     Current Short: {record.get('currentShortMarketSituation', 0):.2f}%")
            print(f"     Previous Market Situation: {record.get('previousMarketSituation', 'N/A')}")
            print(f"     Previous Long: {record.get('previousLongMarketSituation', 0):.2f}%")
            print(f"     Previous Short: {record.get('previousShortMarketSituation', 0):.2f}%")
            print(f"     Net Position: {record.get('netPostion', 0):,}")
            print(f"     Previous Net Position: {record.get('previousNetPosition', 0):,}")
            print(f"     Change in Net Position: {record.get('changeInNetPosition', 0):.1f}%")
    ...
    if gc_analysis:
        ...
        for i, record in enumerate(gc_analysis[:2]):
            ...
            print(f"     Market Sentiment: {record.get('marketSentiment', 'N/A')}")
            print(f"     Long vs Short Ratio: {record.get('currentLongMarketSituation', 0):.2f}% / {record.get('currentShortMarketSituation', 0):.2f}%")
            print()
    ...
    if cot_symbols:
        ...
        for sector, symbols in sectors.items():
            ...
            for i, symbol in enumerate(symbols[:10]):  # Limit to first 10 per sector
                print(f"    {i+1}. {symbol.get('name', 'N/A')} ({symbol.get('symbol', 'N/A')})")
            if len(symbols) > 10:
        ...
        for i, symbol in enumerate(cot_symbols[:15]):
            print(f"  {i+1:2d}. {symbol.get('symbol', 'N/A'):<6} - {symbol.get('name', 'N/A')}")
...
async def cot_market_sentiment_analysis(client: FmpClient) -> None:
    ...
    commodities = ["KC", "NG", "B6", "GC", "SI"]  # Coffee, Natural Gas, British Pound, Gold, Silver
    ...
    for commodity in commodities:
        ...
        try:
            # Get COT analysis
            analysis = await client.cot.cot_analysis(commodity, "2024-01-01", "2024-03-01")
            
            if analysis:
                latest = analysis[0]
                print(f"  Market Situation: {latest.get('marketSituation', 'N/A')}")
                print(f"  Market Sentiment: {latest.get('marketSentiment', 'N/A')}")
                print(f"  Long Position: {latest.get('currentLongMarketSituation', 0):.2f}%")
                print(f"  Short Position: {latest.get('currentShortMarketSituation', 0):.2f}%")
...
async def cot_position_analysis(client: FmpClient) -> None:
    ...
    try:
        ...
        if report:
            ...
            print(f"\n  Non-Commercial Positions:")
            print(f"    Long: {noncomm_long:,} ({noncomm_long/latest.get('openInterestAll', 1)*100:.1f}%)")
            print(f"    Short: {noncomm_short:,} ({noncomm_short/latest.get('openInterestAll', 1)*100:.1f}%)")
            print(f"    Spread: {noncomm_spread:,} ({noncomm_spread/latest.get('openInterestAll', 1)*100:.1f}%)")
            ...
            print(f"\n  Commercial Positions:")
            print(f"    Long: {comm_long:,} ({comm_long/latest.get('openInterestAll', 1)*100:.1f}%)")
            print(f"    Short: {comm_short:,} ({comm_short/latest.get('openInterestAll', 1)*100:.1f}%)")
            ...
            print(f"\n  Total Reportable Positions:")
            print(f"    Long: {total_long:,} ({total_long/latest.get('openInterestAll', 1)*100:.1f}%)")
            print(f"    Short: {total_short:,} ({total_short/latest.get('openInterestAll', 1)*100:.1f}%)")
            
            # Position changes
            print(f"\n  Position Changes:")
            print(f"    Change in Open Interest: {latest.get('changeInOpenInterestAll', 0):+,}")
            print(f"    Change in Non-Commercial Long: {latest.get('changeInNoncommLongAll', 0):+,}")
            print(f"    Change in Non-Commercial Short: {latest.get('changeInNoncommShortAll', 0):+,}")
            print(f"    Change in Commercial Long: {latest.get('changeInCommLongAll', 0):+,}")
            print(f"    Change in Commercial Short: {latest.get('changeInCommShortAll', 0):+,}")
            
            # Trader counts
            print(f"\n  Trader Counts:")
            print(f"    Total Traders: {latest.get('tradersTotAll', 0)}")
            print(f"    Non-Commercial Traders: {latest.get('tradersNoncommLongAll', 0)} long, {latest.get('tradersNoncommShortAll', 0)} short")
            print(f"    Commercial Traders: {latest.get('tradersCommLongAll', 0)} long, {latest.get('tradersCommShortAll', 0)} short")
