...
async def forex_list_examples(client: FmpClient) -> None:
    ...
    if forex_pairs:
        ...
        for currency in major_currencies:
            if currency in base_currencies:
                ...
                for i, pair in enumerate(pairs[:5]):  # Show first 5 pairs
                    print(f"    {i+1}. {pair.get('fromName', 'N/A')} → {pair.get('toName', 'N/A')} ({pair.get('symbol', 'N/A')})")
                if len(pairs) > 5:
        ...
        for i, pair in enumerate(forex_pairs[:10]):
            print(f"  {i+1:2d}. {pair.get('fromCurrency', 'N/A')}/{pair.get('toCurrency', 'N/A')} - {pair.get('fromName', 'N/A')} → {pair.get('toName', 'N/A')}")
...
async def quote_examples(client: FmpClient) -> None:
    ...
    if eur_usd_quote:
        ...
        print(f"  Change: {quote.get('change', 0):+.5f} ({quote.get('changePercentage', 0):+.2f}%)")
        print(f"  Volume: {quote.get('volume', 0):,}")
        print(f"  Day Range: {quote.get('dayLow', 0):.5f} - {quote.get('dayHigh', 0):.5f}")
        print(f"  Year Range: {quote.get('yearLow', 0):.5f} - {quote.get('yearHigh', 0):.5f}")
...
async def quote_short_examples(client: FmpClient) -> None:
    ...
    for symbol in forex_pairs_to_quote:
        ...
        if quote:
            print(f"  {symbol}: {quote[0].get('price', 0):.5f} ({quote[0].get('change', 0):+.5f}) - Vol: {quote[0].get('volume', 0):,}")
        else:
...
async def batch_quotes_examples(client: FmpClient) -> None:
    ...
    if batch_quotes_short:
        ...
        for i, quote in enumerate(batch_quotes_short[:10]):  # Show first 10
            print(f"  {i+1:2d}. {quote.get('symbol', 'N/A'):<10} - {quote.get('price', 0):<12.5f} ({quote.get('change', 0):+.5f})")
...
async def historical_price_examples(client: FmpClient) -> None:
    ...
    eur_usd_light = await client.forex.historical_price_light("EURUSD", "2025-01-01", "2025-01-31")
    print(f"Found {len(eur_usd_light)} historical price records for EUR/USD in January 2025")
    ...
    eur_usd_full = await client.forex.historical_price_full("EURUSD", "2025-01-01", "2025-01-31")
    print(f"Found {len(eur_usd_full)} full historical price records for EUR/USD in January 2025")
    
    if eur_usd_full:
        print("\nEUR/USD Full Historical Prices (January 2025):")
        for i, record in enumerate(eur_usd_full[:3]):  # Show first 3 records
            print(f"  {i+1}. Date: {record.get('date', 'N/A')}")
            print(f"     OHLC: {record.get('open', 0):.5f} / {record.get('high', 0):.5f} / {record.get('low', 0):.5f} / {record.get('close', 0):.5f}")
            print(f"     Volume: {record.get('volume', 0):,}")
            print(f"     Change: {record.get('change', 0):+.5f} ({record.get('changePercent', 0):+.2f}%)")
            print(f"     VWAP: {record.get('vwap', 0):.5f}")
            print()
    
    # Get historical prices for GBP/USD
    print("Fetching historical prices for GBP/USD...")
    gbp_usd_prices = await client.forex.historical_price_light("GBPUSD", "2025-01-01", "2025-01-31")
    print(f"Found {len(gbp_usd_prices)} historical price records for GBP/USD in January 2025")
...
async def intraday_examples(client: FmpClient) -> None:
    ...
    eur_usd_1min = await client.forex.intraday_1min("EURUSD", "2025-01-15", "2025-01-15")
    ...
    eur_usd_5min = await client.forex.intraday_5min("EURUSD", "2025-01-15", "2025-01-15")
    ...
    eur_usd_1hour = await client.forex.intraday_1hour("EURUSD", "2025-01-15", "2025-01-15")
...
async def forex_analysis_examples(client: FmpClient) -> None:
    ...
    if eur_usd_quote and gbp_usd_quote:
        ...
        print(f"    Change: {eur_usd.get('change', 0):+.5f} ({eur_usd.get('changePercentage', 0):+.2f}%)")
        ...
        print(f"    Change: {gbp_usd.get('change', 0):+.5f} ({gbp_usd.get('changePercentage', 0):+.2f}%)")
...
async def forex_portfolio_analysis(client: FmpClient) -> None:
    ...
    # Risk assessment
    print(f"\nRisk Assessment:")
    high_volatility = [sym for sym, data in forex_data.items() if abs(data['change_pct']) > 1]
    if high_volatility:
    ...
    if forex_data:
        ...
        if len(portfolio) >= 4:
            ...
            print(f"  ⚠️  Limited diversification with only {len(portfolio)} currency pairs")
        
        # Check for major vs minor currency exposure
        major_currencies = ['EUR', 'GBP', 'JPY', 'USD']
        major_exposure = sum(data['allocation'] for symbol, data in forex_data.items() 
