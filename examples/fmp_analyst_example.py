...
async def financial_estimates_example(client: FmpClient) -> None:
    ...
    annual_estimates = await client.analyst.financial_estimates("AAPL", "annual", limit=5)
    ...
    quarterly_estimates = await client.analyst.financial_estimates("AAPL", "quarter", limit=3)
...
async def ratings_example(client: FmpClient) -> None:
    ...
    if ratings:
        ...
        print(f"Discounted Cash Flow Score: {rating.get('discountedCashFlowScore', 'N/A')}/5")
...
async def price_targets_example(client: FmpClient) -> None:
    ...
    if price_summary:
        ...
        print(f"  Last Month: {summary.get('lastMonthCount', 0)} analysts, "
              f"Avg: ${summary.get('lastMonthAvgPriceTarget', 0):.2f}")
        print(f"  Last Quarter: {summary.get('lastQuarterCount', 0)} analysts, "
              f"Avg: ${summary.get('lastQuarterAvgPriceTarget', 0):.2f}")
        print(f"  Last Year: {summary.get('lastYearCount', 0)} analysts, "
              f"Avg: ${summary.get('lastYearAvgPriceTarget', 0):.2f}")
        print(f"  All Time: {summary.get('allTimeCount', 0)} analysts, "
    ...
    if price_news:
        ...
        for i, news in enumerate(price_news[:3]):
            ...
            print(f"     Analyst: {news.get('analystName', 'N/A')} ({news.get('analystCompany', 'N/A')})")
...
async def stock_grades_example(client: FmpClient) -> None:
    ...
    if grades_summary:
        ...
        total_ratings = (summary.get('strongBuy', 0) + summary.get('buy', 0) + 
                        summary.get('hold', 0) + summary.get('sell', 0) + 
                        summary.get('strongSell', 0))
        if total_ratings > 0:
            buy_percentage = ((summary.get('strongBuy', 0) + summary.get('buy', 0)) / total_ratings) * 100
...
async def latest_news_example(client: FmpClient) -> None:
    ...
    if latest_price_news:
        ...
        for i, news in enumerate(latest_price_news[:3]):
            print(f"  {i+1}. {news.get('symbol', 'N/A')}: {news.get('newsTitle', 'N/A')}")
            print(f"     Analyst: {news.get('analystName', 'N/A')} ({news.get('analystCompany', 'N/A')})")
            print(f"     Price Target: ${news.get('priceTarget', 0):.2f}")
            print(f"     Publisher: {news.get('newsPublisher', 'N/A')}")
            print(f"     Date: {news.get('publishedDate', 'N/A')}")
...
async def multi_symbol_analysis_example(client: FmpClient) -> None:
    ...
    for symbol in symbols:
        ...
        try:
            ...
            if consensus:
                consensus_data = consensus[0]
                print(f"  Price Target Consensus: ${consensus_data.get('targetConsensus', 0):.2f}")
                print(f"  Price Target Range: ${consensus_data.get('targetLow', 0):.2f} - ${consensus_data.get('targetHigh', 0):.2f}")
