...
async def company_profile_examples(client: FmpClient) -> None:
    ...
    if profile_data:
        ...
        print(f"  Address: {profile.get('address', 'N/A')}, {profile.get('city', 'N/A')}, {profile.get('state', 'N/A')} {profile.get('zip', 'N/A')}")
...
async def employee_data_examples(client: FmpClient) -> None:
    ...
    historical_employee_data = await client.company.historical_employee_count("AAPL", limit=10)
    ...
    if historical_employee_data:
        ...
        for i, record in enumerate(historical_employee_data[:5]):
            print(f"  {i+1}. {record.get('periodOfReport', 'N/A')}: {record.get('employeeCount', 0):,} employees")
...
async def market_cap_examples(client: FmpClient) -> None:
    ...
    if batch_market_cap:
        ...
        for record in batch_market_cap:
            print(f"  {record.get('symbol', 'N/A')}: ${record.get('marketCap', 0):,.0f}")
    ...
    historical_market_cap = await client.company.historical_market_cap("AAPL", from_date=from_date, to_date=to_date)
    ...
    if historical_market_cap:
        ...
        for i, record in enumerate(historical_market_cap[:5]):
            print(f"  {i+1}. {record.get('date', 'N/A')}: ${record.get('marketCap', 0):,.0f}")
...
async def mergers_acquisitions_examples(client: FmpClient) -> None:
    ...
    if latest_ma:
        ...
        for i, record in enumerate(latest_ma[:5]):
            print(f"  {i+1}. {record.get('companyName', 'N/A')} ({record.get('symbol', 'N/A')})")
            print(f"     Target: {record.get('targetedCompanyName', 'N/A')} ({record.get('targetedSymbol', 'N/A')})")
...
async def executives_examples(client: FmpClient) -> None:
    ...
    if benchmark_data:
        ...
        for i, record in enumerate(benchmark_data[:5]):
            ...
            print(f"     Year: {record.get('year', 'N/A')}")
            print(f"     Average Compensation: ${record.get('averageCompensation', 0):,.0f}")
    ...
    for symbol in symbols:
        ...
        try:
            ...
            if market_cap:
                ...
                print(f"  Latest Market Cap: ${latest_market_cap.get('marketCap', 0):,.0f}")
            
            if shares_float:
                latest_shares = shares_float[0]
                print(f"  Free Float: {latest_shares.get('freeFloat', 0):.2f}%")
                print(f"  Outstanding Shares: {latest_shares.get('outstandingShares', 0):,}")
            
            if executives:
                print(f"  Executives: {len(executives)} found")
                if executives:
                    ceo = next((e for e in executives if 'CEO' in e.get('title', '').upper()), None)
                    if ceo:
