    ...
    if commodities:
        ...
        for currency, comm_list in currencies.items():
            ...
            for i, comm in enumerate(comm_list[:5]):  # Show first 5 per currency
                print(f"    {i+1}. {comm.get('name', 'N/A')} ({comm.get('symbol', 'N/A')})")
        ...
        for i, commodity in enumerate(commodities[:10]):
            print(f"  {i+1:2d}. {commodity.get('name', 'N/A'):<25} - {commodity.get('symbol', 'N/A'):<8} ({commodity.get('currency', 'N/A')})")
...
async def quote_short_examples(client: FmpClient) -> None:
    ...
    for symbol in commodities_to_quote:
        ...
        if quote:
            print(f"  {symbol}: ${quote[0].get('price', 0):,.2f} ({quote[0].get('change', 0):+.2f}) - Vol: {quote[0].get('volume', 0):,}")
        else:
...
async def historical_price_examples(client: FmpClient) -> None:
    ...
    gold_light = await client.commodity.historical_price_light("GCUSD", "2025-01-01", "2025-01-31")
    ...
    gold_full = await client.commodity.historical_price_full("GCUSD", "2025-01-01", "2025-01-31")
    print(f"Found {len(gold_full)} full historical price records for Gold in January 2025")
    ...
    silver_prices = await client.commodity.historical_price_light("SIUSD", "2025-01-01", "2025-01-31")
    print(f"Found {len(silver_prices)} historical price records for Silver in January 2025")
...
async def intraday_examples(client: FmpClient) -> None:
    ...
    gold_1min = await client.commodity.intraday_1min("GCUSD", "2025-01-15", "2025-01-15")
    ...
    gold_5min = await client.commodity.intraday_5min("GCUSD", "2025-01-15", "2025-01-15")
    ...
    gold_1hour = await client.commodity.intraday_1hour("GCUSD", "2025-01-15", "2025-01-15")
...
async def commodity_analysis_examples(client: FmpClient) -> None:
    ...
    if gold_quote and silver_quote:
        ...
        print(f"    Change: {gold.get('change', 0):+.2f} ({gold.get('changePercentage', 0):+.2f}%)")
        print(f"    Volume: {gold.get('volume', 0):,}")
        print(f"    50-Day Avg: ${gold.get('priceAvg50', 0):,.2f}")
        print(f"    200-Day Avg: ${gold.get('priceAvg200', 0):,.2f}")
        
        print(f"  Silver (SIUSD):")
        print(f"    Price: ${silver.get('price', 0):,.2f}")
        print(f"    Change: {silver.get('change', 0):+.2f} ({silver.get('changePercentage', 0):+.2f}%)")
    ...
    if oil_quote and gas_quote:
        ...
        print(f"  Crude Oil (CLUSD):")
        print(f"    Price: ${oil.get('price', 0):,.2f}")
        print(f"    Change: {oil.get('change', 0):+.2f} ({oil.get('changePercentage', 0):+.2f}%)")
        print(f"    Day Range: ${oil.get('dayLow', 0):,.2f} - ${oil.get('dayHigh', 0):,.2f}")
        
        print(f"  Natural Gas (NGUSD):")
        print(f"    Price: ${gas.get('price', 0):,.2f}")
        print(f"    Change: {gas.get('change', 0):+.2f} ({gas.get('changePercentage', 0):+.2f}%)")
        print(f"    Day Range: ${gas.get('dayLow', 0):,.2f} - ${gas.get('dayHigh', 0):,.2f}")
...
async def commodity_portfolio_analysis(client: FmpClient) -> None:
    ...
    # Risk assessment
    print(f"\nRisk Assessment:")
    high_volatility = [sym for sym, data in commodity_data.items() if abs(data['change_pct']) > 2]
    if high_volatility:
