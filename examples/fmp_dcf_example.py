    ...
    if dcf_data:
        ...
        for i, record in enumerate(dcf_data):
            ...
            if dcf_value > 0 and stock_price > 0:
                ...
                if upside > 0:
                    print(f"     Investment Thesis: Undervalued")
                elif upside < 0:
                    print(f"     Investment Thesis: Overvalued")
                else:
                    print(f"     Investment Thesis: Fairly Valued")
...
async def custom_dcf_advanced_examples(client: FmpClient) -> None:
    ...
    if custom_dcf:
        ...
        for i, record in enumerate(custom_dcf[:3]):  # Show first 3 years
            ...
            print(f"     Equity Value Per Share: ${record.get('equityValuePerShare', 0):.2f}")
            print()
...
async def custom_dcf_levered_examples(client: FmpClient) -> None:
    ...
    custom_levered = await client.dcf.custom_dcf_levered("AAPL", revenue_growth_pct=0.109, beta=1.244)
    ...
    if custom_levered:
        ...
        for i, record in enumerate(custom_levered[:3]):  # Show first 3 years
            ...
            print(f"     Revenue: ${record.get('revenue', 0):,.0f}")
            print(f"     Revenue Growth: {record.get('revenuePercentage', 0):.2f}%")
            print(f"     Capital Expenditure: ${record.get('capitalExpenditure', 0):,.0f}")
            print(f"     Beta: {record.get('beta', 'N/A')}")
            print(f"     Cost of Debt: {record.get('costofDebt', 'N/A')}%")
            print(f"     Cost of Equity: {record.get('costOfEquity', 'N/A')}%")
            print(f"     WACC: {record.get('wacc', 'N/A')}%")
            print(f"     Operating Cash Flow: ${record.get('operatingCashFlow', 0):,.0f}")
        ...
        try:
            ...
            if basic_dcf and levered_dcf:
                ...
                if basic_value > 0 and levered_value > 0 and stock_price > 0:
                    ...
                    print(f"  Basic DCF Value: ${basic_value:.2f} (Upside: {basic_upside:+.2f}%)")
                    print(f"  Levered DCF Value: ${levered_value:.2f} (Upside: {levered_upside:+.2f}%)")
                    
                    # Determine which method shows higher value
                    if basic_upside > levered_upside:
                        print(f"  Analysis: Basic DCF shows higher upside (debt impact considered)")
                    elif levered_upside > basic_upside:
                        print(f"  Analysis: Levered DCF shows higher upside (debt impact beneficial)")
                    else:
                        print(f"  Analysis: Both methods show similar upside")
            
            # Show custom DCF projections if available
            if custom_dcf:
                print(f"  Custom DCF Projections:")
                for i, record in enumerate(custom_dcf[:3]):  # Show first 3 years
                    year = record.get('year', 'N/A')
                    revenue = record.get('revenue', 0)
                    equity_value = record.get('equityValuePerShare', 0)
                    print(f"    {year}: Revenue ${revenue:,.0f}, Equity Value ${equity_value:.2f}")
    ...
    for growth_rate in growth_rates:
        try:
            ...
            if custom_dcf:
                ...
                print(f"  {growth_rate*100:2.0f}% Growth: Equity Value = ${equity_value:.2f}")
            else:
    ...
    for tax_rate in tax_rates:
        try:
            ...
            if custom_dcf:
                latest = custom_dcf[0]
                equity_value = latest.get('equityValuePerShare', 0)
                print(f"  Tax Rate {tax_rate*100:2.0f}%: Equity Value = ${equity_value:.2f}")
            else:
