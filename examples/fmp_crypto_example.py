    ...
    if cryptocurrencies:
        ...
        for exchange, crypto_list in exchanges.items():
            ...
            for i, crypto in enumerate(crypto_list[:5]):  # Show first 5 per exchange
                print(f"    {i+1}. {crypto.get('name', 'N/A')} ({crypto.get('symbol', 'N/A')})")
                print(f"       ICO Date: {crypto.get('icoDate', 'N/A')}")
                print(f"       Circulating Supply: {crypto.get('circulatingSupply', 0):,}")
        ...
        for i, crypto in enumerate(cryptocurrencies[:10]):
            print(f"  {i+1:2d}. {crypto.get('name', 'N/A'):<25} - {crypto.get('symbol', 'N/A'):<8} ({crypto.get('exchange', 'N/A')})")
...
async def quote_examples(client: FmpClient) -> None:
    ...
    if btc_quote:
        ...
        print(f"  Day Range: ${quote.get('dayLow', 0):,.2f} - ${quote.get('dayHigh', 0):,.2f}")
        print(f"  Year Range: ${quote.get('yearLow', 0):,.2f} - ${quote.get('yearHigh', 0):,.2f}")
    ...
    if ada_quote:
        ...
        print(f"  Change: {quote.get('change', 0):+.4f} ({quote.get('changePercentage', 0):+.2f}%)")
        print(f"  Volume: {quote.get('volume', 0):,.0f}")
        print()
...
async def quote_short_examples(client: FmpClient) -> None:
    ...
    for symbol in cryptocurrencies_to_quote:
        ...
        if quote:
            print(f"  {symbol}: ${quote[0].get('price', 0):,.4f} ({quote[0].get('change', 0):+.4f}) - Vol: {quote[0].get('volume', 0):,.0f}")
        else:
...
async def batch_quotes_examples(client: FmpClient) -> None:
    ...
    if batch_quotes_short:
        ...
        for i, quote in enumerate(batch_quotes_short[:10]):  # Show first 10
            print(f"  {i+1:2d}. {quote.get('symbol', 'N/A'):<10} - ${quote.get('price', 0):<12,.4f} ({quote.get('change', 0):+.4f})")
...
async def historical_price_examples(client: FmpClient) -> None:
    ...
    btc_light = await client.crypto.historical_price_light("BTCUSD", "2025-01-01", "2025-01-31")
    print(f"Found {len(btc_light)} historical price records for Bitcoin in January 2025")
    ...
    btc_full = await client.crypto.historical_price_full("BTCUSD", "2025-01-01", "2025-01-31")
    print(f"Found {len(btc_full)} full historical price records for Bitcoin in January 2025")
    
    if btc_full:
        print("\nBitcoin Full Historical Prices (January 2025):")
        for i, record in enumerate(btc_full[:3]):  # Show first 3 records
            print(f"  {i+1}. Date: {record.get('date', 'N/A')}")
            print(f"     OHLC: ${record.get('open', 0):,.2f} / ${record.get('high', 0):,.2f} / ${record.get('low', 0):,.2f} / ${record.get('close', 0):,.2f}")
            print(f"     Volume: {record.get('volume', 0):,.0f}")
            print(f"     Change: {record.get('change', 0):+.2f} ({record.get('changePercent', 0):+.2f}%)")
            print(f"     VWAP: ${record.get('vwap', 0):,.2f}")
            print()
    
    # Get historical prices for Ethereum
    print("Fetching historical prices for Ethereum (ETHUSD)...")
    eth_prices = await client.crypto.historical_price_light("ETHUSD", "2025-01-01", "2025-01-31")
    print(f"Found {len(eth_prices)} historical price records for Ethereum in January 2025")
...
async def crypto_analysis_examples(client: FmpClient) -> None:
    ...
    if btc_quote and eth_quote:
        ...
        print(f"  Bitcoin (BTCUSD):")
        print(f"    Price: ${btc.get('price', 0):,.2f}")
        print(f"    Change: {btc.get('change', 0):+.2f} ({btc.get('changePercentage', 0):+.2f}%)")
        ...
        print(f"  Ethereum (ETHUSD):")
        print(f"    Price: ${eth.get('price', 0):,.2f}")
        print(f"    Change: {eth.get('change', 0):+.2f} ({eth.get('changePercentage', 0):+.2f}%)")
...
async def crypto_portfolio_analysis(client: FmpClient) -> None:
    ...
    # Risk assessment
    print(f"\nRisk Assessment:")
    high_volatility = [sym for sym, data in crypto_data.items() if abs(data['change_pct']) > 5]
    if high_volatility:
        print(f"  ⚠️  High volatility cryptocurrencies: {', '.join(high_volatility)}")
    
    # Market cap analysis
    if crypto_data:
        total_portfolio_market_cap = sum(data['market_cap'] * data['allocation'] for data in crypto_data.values())
        print(f"\nMarket Cap Analysis:")
        print(f"  Total Portfolio Market Cap: ${total_portfolio_market_cap:,.0f}")
        
        # Check for large cap vs small cap exposure
        large_cap_exposure = sum(data['allocation'] for data in crypto_data.values() if data['market_cap'] > 100000000000)  # >$100B
    ...
    if len(portfolio) >= 4:
        ...
        print(f"  ⚠️  Limited diversification with only {len(portfolio)} cryptocurrencies")
