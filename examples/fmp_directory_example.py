...
async def financial_symbols_example(client: FmpClient) -> None:
    ...
    for currency, count in sorted(currencies.items(), key=lambda x: x[1], reverse=True)[:5]:
...
async def earnings_transcripts_example(client: FmpClient) -> None:
    ...
    sorted_transcripts = sorted(
        transcripts, 
        key=lambda x: int(x.get('noOfTranscripts', 0)), 
        reverse=True
    )
    ...
    for i, company in enumerate(sorted_transcripts[:10]):
        ...
        print(f"  {i+1:2d}. {company['symbol']}: {company.get('companyName', 'N/A')} ({transcript_count} transcripts)")
...
async def available_exchanges_example(client: FmpClient) -> None:
    ...
    for exchange in exchanges:
        ...
        print(f"    Country: {exchange.get('countryName', 'N/A')} ({exchange.get('countryCode', 'N/A')})")
...
async def available_sectors_example(client: FmpClient) -> None:
    ...
    common_sectors = ['Technology', 'Healthcare', 'Financial Services', 'Consumer Cyclical', 'Basic Materials']
...
async def available_industries_example(client: FmpClient) -> None:
    ...
    tech_industries = [i for i in industries if any(tech_word in i['industry'].lower() 
