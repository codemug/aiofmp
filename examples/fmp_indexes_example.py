        ...
        try:
            ...
            if quote:
                ...
                if 'priceAvg50' in data and 'priceAvg200' in data:
                    ...
                    if avg_50 > avg_200:
                        print(f"  📊 Above 200-day average (Bullish)")
                    else:
                        print(f"  📊 Below 200-day average (Bearish)")
...
async def all_index_quotes_examples(client: FmpClient) -> None:
    ...
    if all_quotes:
        ...
        for i, quote in enumerate(all_quotes[:15]):
            ...
            print(f"  {i+1:2d}. {symbol}: ${price:,.2f} ({change:+.2f}) | Vol: {volume:,}")
...
async def market_analysis_examples(client: FmpClient) -> None:
        ...
        try:
            ...
            if quote:
                ...
                if 'priceAvg50' in data and 'priceAvg200' in data:
                    ...
                    if avg_50 > avg_200:
                        print(f"    📊 Above 200-day average (Bullish trend)")
                    else:
                        print(f"    📊 Below 200-day average (Bearish trend)")
                    
                    # Golden/Death cross analysis
                    if avg_50 > avg_200 and price > avg_50:
                        print(f"    🟢 Golden Cross: Strong bullish momentum")
                    elif avg_50 < avg_200 and price < avg_50:
                        print(f"    🔴 Death Cross: Strong bearish momentum")
                ...
                if 'dayLow' in data and 'dayHigh' in data:
                    ...
                    if price > (day_high + day_low) / 2:
                        print(f"    📈 Trading above daily midpoint")
                    else:
                        print(f"    📉 Trading below daily midpoint")
                ...
                if historical:
                    ...
                    if len(prices) >= 2:
                        ...
                        week_change_pct = (week_change / prices[0]) * 100 if prices[0] != 0 else 0
                        
                        print(f"    📅 7-Day Change: {week_change:+.2f} ({week_change_pct:+.2f}%)")
                        
                        # Trend analysis
                        if week_change > 0:
                            print(f"    📈 Weekly trend: Bullish")
                        elif week_change < 0:
                            print(f"    📉 Weekly trend: Bearish")
                        else:
                            print(f"    ➡️  Weekly trend: Sideways")
    ...
    try:
        ...
        if all_quotes:
            ...
            print(f"  🟢 Advancing: {positive_changes} ({positive_changes/total_indexes*100:.1f}%)")
            print(f"  🔴 Declining: {negative_changes} ({negative_changes/total_indexes*100:.1f}%)")
            print(f"  ⚪ Unchanged: {no_changes} ({no_changes/total_indexes*100:.1f}%)")
