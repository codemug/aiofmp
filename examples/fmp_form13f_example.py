...
async def latest_filings_examples(client: FmpClient) -> None:
    ...
    if latest_filings:
        ...
        # Find recent filings
        recent_filings = [f for f in latest_filings if f.get('date', '') >= '2024-12-01']
...
async def filings_extract_examples(client: FmpClient) -> None:
    ...
    if berkshire_holdings:
        ...
        sorted_holdings = sorted(berkshire_holdings, key=lambda x: x.get('value', 0), reverse=True)
...
async def analytics_by_holder_examples(client: FmpClient) -> None:
    ...
    apple_analytics = await client.form13f.filings_extract_analytics_by_holder("AAPL", "2023", "3", page=0, limit=20)
    ...
    if apple_analytics:
        ...
        sorted_holders = sorted(apple_analytics, key=lambda x: x.get('ownership', 0), reverse=True)
        ...
        significant_changes = [h for h in apple_analytics if abs(h.get('changeInSharesNumberPercentage', 0)) > 10]
...
async def holder_performance_examples(client: FmpClient) -> None:
    ...
    berkshire_performance = await client.form13f.holder_performance_summary("0001067983", page=0)
    print(f"Found {len(berkshire_performance)} performance records for Berkshire Hathaway")
    ...
    vanguard_performance = await client.form13f.holder_performance_summary("0000102909", page=0)
...
async def industry_breakdown_examples(client: FmpClient) -> None:
    ...
    berkshire_industries = await client.form13f.holder_industry_breakdown("0001067983", "2023", "3")
    print(f"Found {len(berkshire_industries)} industry allocations for Berkshire Hathaway")
    
    if berkshire_industries:
        print("\nBerkshire Hathaway Industry Breakdown (Q3 2023):")
        
        # Sort by weight
        sorted_industries = sorted(berkshire_industries, key=lambda x: x.get('weight', 0), reverse=True)
        ...
        total_weight = sum(industry.get('weight', 0) for industry in berkshire_industries)
        print(f"\n  Total Industry Allocation: {total_weight:.2f}%")
        
        # Find top performing industries
        top_performing = sorted(berkshire_industries, key=lambda x: x.get('performance', 0), reverse=True)
...
async def industry_performance_examples(client: FmpClient) -> None:
    ...
    industry_performance = await client.form13f.industry_performance_summary("2023", "3")
    ...
    if industry_performance:
        ...
        sorted_industries = sorted(industry_performance, key=lambda x: x.get('industryValue', 0), reverse=True)
        
        print(f"  Top 10 Industries by Value:")
        ...
        total_value = sum(industry.get('industryValue', 0) for industry in industry_performance)
...
async def institutional_analysis_examples(client: FmpClient) -> None:
    ...
    for symbol in companies:
        ...
        try:
            ...
            positions = await client.form13f.symbol_positions_summary(symbol, year, quarter)
            ...
            if positions:
                ...
                # Get top holders
                analytics = await client.form13f.filings_extract_analytics_by_holder(symbol, year, quarter, page=0, limit=5)
                
                if analytics:
                    print(f"    Top 5 Institutional Holders:")
                    for i, holder in enumerate(analytics[:5]):
                        name = holder.get('investorName', 'N/A')
                        ownership_pct = holder.get('ownership', 0)
                        value = holder.get('marketValue', 0)
                        print(f"      {i+1}. {name}: {ownership_pct:.2f}% (${value:,})")
                ...
                print(f"    Position Changes:")
                print(f"      New: {new_pos} | Increased: {increased} | Reduced: {reduced} | Closed: {closed}")
                
                # Sentiment analysis
                if new_pos > closed and increased > reduced:
                    print(f"      🟢 Bullish institutional sentiment")
                elif closed > new_pos and reduced > increased:
                    print(f"      🔴 Bearish institutional sentiment")
                else:
                    print(f"      🟡 Mixed institutional sentiment")
    ...
    for cik, name in major_investors:
        ...
        try:
            ...
            if performance:
                ...
                print(f"    Portfolio Size: {portfolio_size} positions")
                print(f"    Market Value: ${market_value:,}")
                print(f"    Performance: ${performance_val:,} ({performance_pct:+.2f}%)")
                
                # Get industry breakdown
                industries = await client.form13f.holder_industry_breakdown(cik, year, quarter)
