...
async def treasury_rates_examples(client: FmpClient) -> None:
    ...
    print(f"Fetching treasury rates for last 30 days ({start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')})...")
    recent_rates = await client.economics.treasury_rates(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))
...
async def economic_indicators_examples(client: FmpClient) -> None:
    ...
    gdp_data = await client.economics.economic_indicators("GDP", "2024-01-01", "2024-12-31")
    ...
    cpi_data = await client.economics.economic_indicators("CPI", "2024-01-01", "2024-12-31")
    ...
    unemployment_data = await client.economics.economic_indicators("unemploymentRate", "2024-01-01", "2024-12-31")
    ...
    fed_funds_data = await client.economics.economic_indicators("federalFunds", "2024-01-01", "2024-12-31")
    ...
    inflation_data = await client.economics.economic_indicators("inflationRate", "2024-01-01", "2024-12-31")
...
async def economic_calendar_examples(client: FmpClient) -> None:
    ...
    print(f"Fetching economic calendar for next 30 days ({start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')})...")
    calendar_data = await client.economics.economic_calendar(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))
    ...
    march_calendar = await client.economics.economic_calendar("2024-03-01", "2024-03-31")
    ...
    if risk_premium_data:
        ...
        for continent, countries in continents.items():
            ...
            for i, country in enumerate(countries[:5]):  # Show first 5 per continent
                ...
                print(f"       Country Risk Premium: {country.get('countryRiskPremium', 'N/A')}%")
                print(f"       Total Equity Risk Premium: {country.get('totalEquityRiskPremium', 'N/A')}%")
            if len(countries) > 5:
        ...
        for i, record in enumerate(risk_premium_data[:10]):
            print(f"  {i+1:2d}. {record.get('country', 'N/A'):<20} - CRP: {record.get('countryRiskPremium', 'N/A'):>6}%, TERP: {record.get('totalEquityRiskPremium', 'N/A'):>6}%")
...
async def economic_analysis_examples(client: FmpClient) -> None:
    ...
    for indicator in indicators:
        try:
            data = await client.economics.economic_indicators(indicator, "2024-01-01", "2024-12-31")
            if data:
                ...
                if len(data) >= 2:
                    ...
                    if isinstance(first_value, (int, float)) and isinstance(last_value, (int, float)):
                        if first_value != 0:
                            ...
                            print(f"    Change: {first_value} → {last_value} ({change_pct:+.2f}%)")
                        else:
...
async def economic_calendar_impact_analysis(client: FmpClient) -> None:
    ...
    print(f"Analyzing economic calendar impact for next 7 days ({start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')})...")
    ...
    if calendar_data:
        ...
        for impact, events in impact_levels.items():
            ...
            for i, event in enumerate(events[:3]):  # Show first 3 per impact level
                print(f"    {i+1}. {event.get('date', 'N/A')} - {event.get('country', 'N/A')}: {event.get('event', 'N/A')}")
            if len(events) > 3:
        ...
        if high_impact:
            ...
            for i, event in enumerate(high_impact[:5]):
                print(f"  {i+1}. {event.get('date', 'N/A')} - {event.get('country', 'N/A')}")
                print(f"     Event: {event.get('event', 'N/A')}")
                print(f"     Currency: {event.get('currency', 'N/A')}")
                print(f"     Previous: {event.get('previous', 'N/A')}")
                print(f"     Estimate: {event.get('estimate', 'N/A')}")
