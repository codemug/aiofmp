...
async def get_latest_insider_trades(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved latest insider trading activity"
        )
...
async def get_insider_trade_statistics(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        client = get_fmp_client()
        async with client:
            results = await client.insider_trades.insider_trade_statistics(symbol=validated_symbol)
...
async def search_insider_trades(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved insider trades matching search criteria"
        )
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving insider trades: {str(e)}"
        )
