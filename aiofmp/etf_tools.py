...
async def get_etf_holdings(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving ETF holdings: {str(e)}"
        )
...
async def get_etf_info(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving ETF info: {str(e)}"
        )
...
async def get_etf_asset_exposure(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving asset exposure: {str(e)}"
        )
...
async def get_etf_disclosure_holders_latest(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        client = get_fmp_client()
        async with client:
            results = await client.etf.disclosure_holders_latest(symbol=validated_symbol)
