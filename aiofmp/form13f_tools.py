...
async def get_latest_filings(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved latest Form 13F filings"
        )
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving latest filings: {str(e)}"
        )


@mcp.tool
async def get_filings_extract(
    cik: str,
    year: str,
    quarter: str
) -> Dict[str, Any]:
...
async def get_filings_dates(
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving filing dates: {str(e)}"
        )
