...
async def get_simple_moving_average(
    ...
    try:
        ...
        if not timeframe or not isinstance(timeframe, str) or timeframe not in valid_timeframes:
...
async def get_double_exponential_moving_average(
    ...
    try:
        ...
        client = get_fmp_client()
        async with client:
            results = await client.technical_indicators.double_exponential_moving_average(
                symbol=validated_symbol,
                period_length=period_length,
                timeframe=timeframe,
                from_date=validated_from_date,
...
async def get_triple_exponential_moving_average(
    ...
    try:
        ...
        client = get_fmp_client()
        async with client:
            results = await client.technical_indicators.triple_exponential_moving_average(
                symbol=validated_symbol,
                period_length=period_length,
                timeframe=timeframe,
                from_date=validated_from_date,
...
async def get_williams_percent_r(
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving Williams %R: {str(e)}"
        )
