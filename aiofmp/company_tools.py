    ...
    try:
        ...
        if results and len(results) > 0:
            ...
            if "marketCap" in profile:
                profile["market_cap_formatted"] = format_large_number(profile["marketCap"])
            if "price" in profile:
...
async def get_company_notes(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving company notes: {str(e)}"
        )
    ...
    try:
        ...
        if results and len(results) > 0:
            ...
            if "marketCap" in market_cap:
                market_cap["market_cap_formatted"] = format_large_number(market_cap["marketCap"])
    ...
    try:
        ...
        for result in results:
            if "marketCap" in result:
                result["market_cap_formatted"] = format_large_number(result["marketCap"])
...
async def get_latest_mergers_acquisitions(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved latest mergers and acquisitions data"
        )
...
async def get_executive_compensation(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        client = get_fmp_client()
        async with client:
            results = await client.company.executive_compensation(symbol=validated_symbol)
...
async def get_executive_compensation_benchmark(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved executive compensation benchmark data"
        )
