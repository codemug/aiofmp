...
async def get_price_target_consensus(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        client = get_fmp_client()
        async with client:
            results = await client.analyst.price_target_consensus(symbol=validated_symbol)
...
async def get_stock_grades(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving stock grades: {str(e)}"
        )
...
async def get_price_target_latest_news(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved latest price target news"
        )
...
async def get_stock_grade_latest_news(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved latest stock grade news"
        )
