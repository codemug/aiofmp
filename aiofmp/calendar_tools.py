...
@mcp.tool
async def get_earnings_company(
    symbol: str,
    limit: Optional[int] = None
) -> Dict[str, Any]:
    """
...
async def get_ipos_calendar(
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving IPO calendar: {str(e)}"
        )
...
async def get_ipos_disclosure(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved IPO disclosure filings"
        )
...
async def get_ipos_prospectus(
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved IPO prospectus information"
        )
