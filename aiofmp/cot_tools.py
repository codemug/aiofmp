...
async def get_cot_report(
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving COT report: {str(e)}"
        )
...
async def get_cot_analysis(
    ...
    try:
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving COT analysis: {str(e)}"
        )
...
async def get_cot_list() -> Dict[str, Any]:
    ...
    try:
        ...
        return create_tool_response(
            data=results,
            success=True,
            message=f"Retrieved {len(results)} COT symbols"
        )
        ...
        return create_tool_response(
            data=[],
            success=False,
            message=f"Error retrieving COT list: {str(e)}"
        )
