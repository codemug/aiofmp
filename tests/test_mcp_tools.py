...
from aiofmp.mcp_tools import create_tool_response, validate_symbol, validate_date, validate_limit
from aiofmp.search_tools import search_symbols, search_companies, screen_stocks
...
class TestSearchTools:
    ...
    async def test_search_symbols_success(self):
        ...
        with patch('aiofmp.search_tools.get_fmp_client') as mock_get_client:
            ...
            assert "Found 1 symbols matching 'AAPL'" in result["message"]
    ...
    async def test_search_symbols_error(self):
        ...
        with patch('aiofmp.search_tools.get_fmp_client') as mock_get_client:
            ...
            assert result["data"] == []
            assert "Error searching symbols" in result["message"]
    ...
    async def test_search_companies_success(self):
        ...
        with patch('aiofmp.search_tools.get_fmp_client') as mock_get_client:
            ...
            assert result["success"] is True
            assert result["data"] == mock_data
            assert "Found 1 companies matching 'Apple'" in result["message"]
    
    @pytest.mark.asyncio
    async def test_screen_stocks_success(self):
        """Test successful stock screening."""
        mock_data = [{"symbol": "AAPL", "sector": "Technology", "marketCap": 3000000000000}]
        ...
        with patch('aiofmp.search_tools.get_fmp_client') as mock_get_client:
            ...
            result = await screen_stocks.fn(sector="Technology", market_cap_more_than=1000000000, limit=100)
            
            assert result["success"] is True
            assert result["data"] == mock_data
            assert "Found 1 stocks matching screening criteria" in result["message"]
...
class TestChartTools:
    ...
    async def test_get_historical_price_full_success(self):
        ...
        mock_data = [{"date": "2025-01-01", "open": 150.0, "high": 155.0, "low": 149.0, "close": 154.0}]
        ...
        with patch('aiofmp.chart_tools.get_fmp_client') as mock_get_client:
            ...
            result = await get_historical_price_full.fn("AAPL", "2025-01-01", "2025-01-31")
            
            assert result["success"] is True
            assert result["data"] == mock_data
            assert "Retrieved 1 comprehensive historical price records for AAPL" in result["message"]
    
    @pytest.mark.asyncio
    async def test_get_intraday_1min_success(self):
        """Test successful intraday data retrieval."""
        mock_data = [{"date": "2025-01-01 09:30:00", "open": 150.0, "high": 151.0, "low": 149.0, "close": 150.5}]
        ...
        with patch('aiofmp.chart_tools.get_fmp_client') as mock_get_client:
            ...
            assert result["success"] is True
            assert result["data"] == mock_data
            assert "Retrieved 1 1-minute intraday records for AAPL" in result["message"]
...
class TestCompanyTools:
    ...
    async def test_get_company_profile_success(self):
        ...
        mock_data = [{"symbol": "AAPL", "companyName": "Apple Inc.", "sector": "Technology", "marketCap": 3000000000000}]
        ...
        with patch('aiofmp.company_tools.get_fmp_client') as mock_get_client:
            ...
            assert result["success"] is True
            assert result["data"] == mock_data
            assert "Retrieved company profile for AAPL" in result["message"]
    
    @pytest.mark.asyncio
    async def test_get_market_cap_success(self):
        """Test successful market cap retrieval."""
        mock_data = [{"symbol": "AAPL", "marketCap": 3000000000000, "date": "2025-01-01"}]
        ...
        with patch('aiofmp.company_tools.get_fmp_client') as mock_get_client:
            ...
            assert result["success"] is True
            assert result["data"] == mock_data
            assert "Retrieved market cap data for AAPL" in result["message"]
...
class TestMCPToolsIntegration:
    ...
    async def test_tool_validation(self):
        ...
        with patch('aiofmp.search_tools.get_fmp_client') as mock_get_client:
            ...
            assert result["success"] is False
            assert "Query must be a non-empty string" in result["message"]
