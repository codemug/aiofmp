...
class TestForm13FCategory:
    ...
    async def test_latest_filings_basic(self, form13f_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with("institutional-ownership/latest", {})
    
    @pytest.mark.asyncio
    async def test_latest_filings_with_optional_params(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_filings_extract_analytics_by_holder_basic(self, form13f_category, mock_client):
        ...
        result = await form13f_category.filings_extract_analytics_by_holder("AAPL", "2023", "3")
    ...
    @pytest.mark.asyncio
    async def test_filings_extract_analytics_by_holder_with_optional_params(self, form13f_category, mock_client):
        ...
        result = await form13f_category.filings_extract_analytics_by_holder("AAPL", "2023", "3", page=0, limit=10)
    ...
    @pytest.mark.asyncio
    async def test_holder_performance_summary_basic(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_holder_performance_summary_with_page(self, form13f_category, mock_client):
        """Test holder performance summary with page parameter"""
        mock_response = [{"investorName": "BERKSHIRE HATHAWAY INC", "portfolioSize": 40}]
        mock_client._make_request.return_value = mock_response
    ...
    async def test_holder_industry_breakdown_basic(self, form13f_category, mock_client):
        ...
        result = await form13f_category.holder_industry_breakdown("0001067983", "2023", "3")
    ...
    @pytest.mark.asyncio
    async def test_industry_performance_summary_basic(self, form13f_category, mock_client):
    ...
    async def test_parameter_combinations(self, form13f_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_with("institutional-ownership/latest", {"page": 0})
        
        # Test with only limit
        result = await form13f_category.latest_filings(limit=50)
        assert result == mock_response
        mock_client._make_request.assert_called_with("institutional-ownership/latest", {"limit": 50})
        
        # Test with both parameters
        result = await form13f_category.latest_filings(page=1, limit=25)
        assert result == mock_response
        mock_client._make_request.assert_called_with("institutional-ownership/latest", {"page": 1, "limit": 25})
    
    @pytest.mark.asyncio
    async def test_different_ciks(self, form13f_category, mock_client):
        """Test Form 13F functionality with different CIKs"""
        mock_response = [{"cik": "0001067983", "investorName": "BERKSHIRE HATHAWAY INC"}]
        mock_client._make_request.return_value = mock_response
        
        # Test with Berkshire Hathaway
        result = await form13f_category.filings_dates("0001067983")
        assert result == mock_response
        mock_client._make_request.assert_called_with("institutional-ownership/dates", {"cik": "0001067983"})
        
        # Test with Vanguard
        result = await form13f_category.filings_dates("0000102909")
        assert result == mock_response
        mock_client._make_request.assert_called_with("institutional-ownership/dates", {"cik": "0000102909"})
        
        # Test with BlackRock
        result = await form13f_category.filings_dates("0001100663")
        assert result == mock_response
        mock_client._make_request.assert_called_with("institutional-ownership/dates", {"cik": "0001100663"})
    ...
    @pytest.mark.asyncio
    async def test_latest_filings_response_validation(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_filings_extract_response_validation(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_filings_dates_response_validation(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_analytics_by_holder_response_validation(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_performance_summary_response_validation(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_industry_breakdown_response_validation(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_symbol_positions_summary_response_validation(self, form13f_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_industry_performance_summary_response_validation(self, form13f_category, mock_client):
        ...
        assert len(result) == 1
        assert result[0]["industryTitle"] == "ABRASIVE, ASBESTOS & MISC NONMETALLIC MINERAL PRODS"
