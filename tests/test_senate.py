...
class TestSenateCategory:
    ...
    @pytest.mark.asyncio
    async def test_latest_senate_disclosures_with_pagination(self, senate_category, mock_client):
        """Test latest Senate disclosures with pagination parameters"""
        mock_response = [{"symbol": "LRN", "firstName": "Markwayne", "lastName": "Mullin"}]
        mock_client._make_request.return_value = mock_response
        
        result = await senate_category.latest_senate_disclosures(page=1, limit=50)
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("senate-latest", {"page": 1, "limit": 50})
    
    @pytest.mark.asyncio
    async def test_latest_senate_disclosures_page_only(self, senate_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_latest_senate_disclosures_limit_only(self, senate_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("senate-latest", {"limit": 25})
    ...
    @pytest.mark.asyncio
    async def test_latest_house_disclosures_with_pagination(self, senate_category, mock_client):
        """Test latest House disclosures with pagination parameters"""
        mock_response = [{"symbol": "$VIRTUALUSD", "firstName": "Michael", "lastName": "Collins"}]
        mock_client._make_request.return_value = mock_response
        
        result = await senate_category.latest_house_disclosures(page=0, limit=75)
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("house-latest", {"page": 0, "limit": 75})
    ...
    async def test_senate_trading_activity_basic(self, senate_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("senate-trades", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_senate_trading_activity_different_symbol(self, senate_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("senate-trades", {"symbol": "MSFT"})
    ...
    async def test_senate_trading_activity_empty(self, senate_category, mock_client):
        ...
        assert result == []
        mock_client._make_request.assert_called_once_with("senate-trades", {"symbol": "UNKNOWN"})
    ...
    async def test_senate_trades_by_name_basic(self, senate_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("senate-trades-by-name", {"name": "Jerry"})
    
    @pytest.mark.asyncio
    async def test_senate_trades_by_name_different_name(self, senate_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("senate-trades-by-name", {"name": "Jane"})
    ...
    async def test_senate_trades_by_name_empty(self, senate_category, mock_client):
        ...
        assert result == []
        mock_client._make_request.assert_called_once_with("senate-trades-by-name", {"name": "Unknown"})
    ...
    async def test_house_trading_activity_basic(self, senate_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("house-trades", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_house_trading_activity_different_symbol(self, senate_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("house-trades", {"symbol": "GOOGL"})
    ...
    async def test_house_trading_activity_empty(self, senate_category, mock_client):
        ...
        assert result == []
        mock_client._make_request.assert_called_once_with("house-trades", {"symbol": "UNKNOWN"})
    ...
    async def test_house_trades_by_name_basic(self, senate_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("house-trades-by-name", {"name": "James"})
    
    @pytest.mark.asyncio
    async def test_house_trades_by_name_different_name(self, senate_category, mock_client):
        """Test House trades by name with different name"""
        mock_response = [{"symbol": "NVDA", "firstName": "Sarah", "lastName": "Johnson"}]
        mock_client._make_request.return_value = mock_response
        
        result = await senate_category.house_trades_by_name("Sarah")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("house-trades-by-name", {"name": "Sarah"})
    ...
    async def test_house_trades_by_name_empty(self, senate_category, mock_client):
        ...
        assert result == []
        mock_client._make_request.assert_called_once_with("house-trades-by-name", {"name": "Unknown"})
    
    @pytest.mark.asyncio
    async def test_large_senate_disclosures_response(self, senate_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with("senate-latest", {"page": 0, "limit": 100})
    ...
    async def test_large_house_disclosures_response(self, senate_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with("house-latest", {"page": 1, "limit": 75})
    ...
    async def test_different_symbols_consistency(self, senate_category, mock_client):
        ...
        for symbol in symbols:
            ...
            mock_client._make_request.assert_called_with("senate-trades", {"symbol": symbol})
            ...
            assert result[0]["symbol"] == symbol
            mock_client._make_request.assert_called_with("house-trades", {"symbol": symbol})
    ...
    async def test_different_names_consistency(self, senate_category, mock_client):
        ...
        for name in names:
            ...
            mock_client._make_request.assert_called_with("senate-trades-by-name", {"name": name})
            ...
            assert result[0]["firstName"] == name
            mock_client._make_request.assert_called_with("house-trades-by-name", {"name": name})
    ...
    async def test_pagination_edge_cases(self, senate_category, mock_client):
        ...
        result = await senate_category.latest_senate_disclosures(page=0, limit=100)
        assert result == mock_response
        mock_client._make_request.assert_called_with("senate-latest", {"page": 0, "limit": 100})
        
        # Test with large page number
        result = await senate_category.latest_senate_disclosures(page=999, limit=50)
        assert result == mock_response
        mock_client._make_request.assert_called_with("senate-latest", {"page": 999, "limit": 50})
        
        # Test with large limit
        result = await senate_category.latest_senate_disclosures(page=1, limit=1000)
        assert result == mock_response
        mock_client._make_request.assert_called_with("senate-latest", {"page": 1, "limit": 1000})
    ...
    async def test_method_consistency(self, senate_category, mock_client):
        ...
        for method_name, endpoint in methods:
            ...
            if "disclosures" in method_name:
                ...
                assert result == mock_response
                mock_client._make_request.assert_called_with(endpoint, {"page": 0, "limit": 100})
                ...
                if "senate" in method_name and "name" in method_name:
                    ...
                    assert result == mock_response
                    mock_client._make_request.assert_called_with(endpoint, {"name": "Jerry"})
                elif "house" in method_name and "name" in method_name:
                    result = await method("James")
                    assert result == mock_response
                    mock_client._make_request.assert_called_with(endpoint, {"name": "James"})
                else:
    ...
    @pytest.mark.asyncio
    async def test_senate_disclosures_response_validation(self, senate_category, mock_client):
        ...
        assert result[0]["link"] == "https://efdsearch.senate.gov/search/view/ptr/446c7588-5f97-42c0-8983-3ca975b91793/"
        mock_client._make_request.assert_called_once_with("senate-latest", {})
    
    @pytest.mark.asyncio
    async def test_house_disclosures_response_validation(self, senate_category, mock_client):
        ...
        assert result[0]["comment"] == ""
        assert result[0]["link"] == "https://disclosures-clerk.house.gov/public_disc/ptr-pdfs/2025/20026696.pdf"
        mock_client._make_request.assert_called_once_with("house-latest", {})
