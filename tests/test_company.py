...
class TestCompanyCategory:
    ...
    async def test_notes_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("company-notes", {"symbol": "AAPL"})
    ...
    async def test_employee_count_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("employee-count", {"symbol": "AAPL"})
    ...
    async def test_employee_count_with_limit(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("employee-count", {"symbol": "AAPL", "limit": 10})
    ...
    async def test_historical_employee_count_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-employee-count", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_historical_employee_count_with_limit(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-employee-count", {"symbol": "AAPL", "limit": 20})
    ...
    async def test_market_cap_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("market-capitalization", {"symbol": "AAPL"})
    ...
    async def test_batch_market_cap_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("market-capitalization-batch", {"symbols": "AAPL,MSFT,GOOGL"})
    ...
    async def test_historical_market_cap_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-market-capitalization", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_historical_market_cap_with_all_params(self, company_category, mock_client):
        """Test historical market cap with all parameters"""
        mock_response = [{"symbol": "AAPL", "date": "2024-02-29", "marketCap": 2784608472000}]
        mock_client._make_request.return_value = mock_response
        
        result = await company_category.historical_market_cap("AAPL", limit=100, from_date="2025-01-01", to_date="2025-03-31")
    ...
    async def test_shares_float_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("shares-float", {"symbol": "AAPL"})
    ...
    async def test_all_shares_float_with_params(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("shares-float-all", {"limit": 1000, "page": 0})
    
    @pytest.mark.asyncio
    async def test_latest_mergers_acquisitions_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("mergers-acquisitions-latest", {})
    
    @pytest.mark.asyncio
    async def test_latest_mergers_acquisitions_with_params(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("mergers-acquisitions-latest", {"page": 0, "limit": 100})
    
    @pytest.mark.asyncio
    async def test_search_mergers_acquisitions_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("mergers-acquisitions-search", {"name": "Apple"})
    ...
    async def test_executives_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("key-executives", {"symbol": "AAPL"})
    ...
    async def test_executives_with_active(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("key-executives", {"symbol": "AAPL", "active": "true"})
    ...
    async def test_executive_compensation_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("governance-executive-compensation", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_executive_compensation_benchmark_basic(self, company_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("executive-compensation-benchmark", {})
    
    @pytest.mark.asyncio
    async def test_executive_compensation_benchmark_with_year(self, company_category, mock_client):
        """Test executive compensation benchmark with year parameter"""
        mock_response = [{"industryTitle": "TECHNOLOGY", "year": 2024, "averageCompensation": 800000}]
        mock_client._make_request.return_value = mock_response
        
        result = await company_category.executive_compensation_benchmark("2024")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("executive-compensation-benchmark", {"year": "2024"})
    ...
    async def test_large_response_handling(self, company_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with("shares-float-all", {"limit": 100})
    ...
    async def test_parameter_combinations(self, company_category, mock_client):
        ...
        result = await company_category.employee_count("AAPL", limit=10)
        assert result == mock_response
        mock_client._make_request.assert_called_with("employee-count", {"symbol": "AAPL", "limit": 10})
        
        # Test with only active
        result = await company_category.executives("AAPL", active="true")
        assert result == mock_response
        mock_client._make_request.assert_called_with("key-executives", {"symbol": "AAPL", "active": "true"})
        
        # Test with only year
        result = await company_category.executive_compensation_benchmark("2024")
        assert result == mock_response
        mock_client._make_request.assert_called_with("executive-compensation-benchmark", {"year": "2024"})
    ...
    async def test_date_parameter_combinations(self, company_category, mock_client):
        ...
        result = await company_category.historical_market_cap("AAPL", from_date="2025-01-01")
        assert result == mock_response
        mock_client._make_request.assert_called_with(
            "historical-market-capitalization",
            {"symbol": "AAPL", "from": "2025-01-01"}
        )
        
        # Test with only to_date
        result = await company_category.historical_market_cap("AAPL", to_date="2025-03-31")
        assert result == mock_response
        mock_client._make_request.assert_called_with(
            "historical-market-capitalization",
            {"symbol": "AAPL", "to": "2025-03-31"}
        )
        
        # Test with both dates
        result = await company_category.historical_market_cap("AAPL", from_date="2025-01-01", to_date="2025-03-31")
        assert result == mock_response
        mock_client._make_request.assert_called_with(
            "historical-market-capitalization",
