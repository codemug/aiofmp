...
class TestForexCategory:
    ...
    async def test_quote_short_basic(self, forex_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("quote-short", {"symbol": "EURUSD"})
    ...
    async def test_quote_short_response_structure(self, forex_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with("quote-short", {"symbol": "USDJPY"})
    ...
    async def test_batch_quotes_default(self, forex_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("batch-forex-quotes", {"short": True})
    
    @pytest.mark.asyncio
    async def test_batch_quotes_with_short_false(self, forex_category, mock_client):
        """Test batch quotes with short=False"""
        mock_response = [{"symbol": "EURUSD", "price": 1.17598, "change": -0.0017376, "volume": 184065}]
        mock_client._make_request.return_value = mock_response
        
        result = await forex_category.batch_quotes(short=False)
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("batch-forex-quotes", {"short": False})
    ...
    async def test_historical_price_light_basic(self, forex_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-price-eod/light", {"symbol": "EURUSD"})
    ...
    async def test_historical_price_light_with_dates(self, forex_category, mock_client):
        ...
        mock_response = [{"symbol": "EURUSD", "date": "2025-07-24", "price": 1.17639, "volume": 182290}]
        mock_client._make_request.return_value = mock_response
        
        result = await forex_category.historical_price_light("EURUSD", "2025-04-25", "2025-07-25")
    ...
    @pytest.mark.asyncio
    async def test_historical_price_light_with_from_date_only(self, forex_category, mock_client):
        """Test historical price light with only from_date parameter"""
        mock_response = [{"symbol": "EURUSD", "date": "2025-07-24", "price": 1.17639}]
        mock_client._make_request.return_value = mock_response
        
        result = await forex_category.historical_price_light("EURUSD", from_date="2025-04-25")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "historical-price-eod/light",
            {"symbol": "EURUSD", "from": "2025-04-25"}
        )
    
    @pytest.mark.asyncio
    async def test_historical_price_light_with_to_date_only(self, forex_category, mock_client):
        """Test historical price light with only to_date parameter"""
        mock_response = [{"symbol": "EURUSD", "date": "2025-07-24", "price": 1.17639}]
        mock_client._make_request.return_value = mock_response
        
        result = await forex_category.historical_price_light("EURUSD", to_date="2025-07-25")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "historical-price-eod/light",
            {"symbol": "EURUSD", "to": "2025-07-25"}
        )
    ...
    async def test_historical_price_full_basic(self, forex_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-price-eod/full", {"symbol": "EURUSD"})
    ...
    async def test_historical_price_full_with_dates(self, forex_category, mock_client):
        ...
        mock_response = [{"symbol": "EURUSD", "date": "2025-07-24", "open": 1.17744, "close": 1.17639}]
        mock_client._make_request.return_value = mock_response
        
        result = await forex_category.historical_price_full("EURUSD", "2025-04-25", "2025-07-25")
    ...
    async def test_intraday_1min_basic(self, forex_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/1min", {"symbol": "EURUSD"})
    
    @pytest.mark.asyncio
    async def test_intraday_1min_with_dates(self, forex_category, mock_client):
        """Test intraday 1min with date parameters"""
        mock_response = [{"date": "2025-07-24 12:29:00", "open": 1.17582, "close": 1.17598}]
        mock_client._make_request.return_value = mock_response
        
        result = await forex_category.intraday_1min("EURUSD", "2025-04-25", "2025-07-25")
    ...
    async def test_intraday_5min_basic(self, forex_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/5min", {"symbol": "EURUSD"})
    
    @pytest.mark.asyncio
    async def test_intraday_5min_with_dates(self, forex_category, mock_client):
        """Test intraday 5min with date parameters"""
        mock_response = [{"date": "2025-07-24 12:25:00", "open": 1.17612, "close": 1.17578}]
        mock_client._make_request.return_value = mock_response
        
        result = await forex_category.intraday_5min("EURUSD", "2025-04-25", "2025-07-25")
    ...
    async def test_intraday_1hour_basic(self, forex_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/1hour", {"symbol": "EURUSD"})
    
    @pytest.mark.asyncio
    async def test_intraday_1hour_with_dates(self, forex_category, mock_client):
        """Test intraday 1hour with date parameters"""
        mock_response = [{"date": "2025-07-24 12:00:00", "open": 1.17639, "close": 1.17578}]
        mock_client._make_request.return_value = mock_response
        
        result = await forex_category.intraday_1hour("EURUSD", "2025-04-25", "2025-07-25")
    ...
    @pytest.mark.asyncio
    async def test_historical_price_full_response_validation(self, forex_category, mock_client):
