...
class TestCalendarCategory:
    ...
    @pytest.mark.asyncio
    async def test_stock_splits_company_with_limit(self, calendar_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_stock_splits_calendar_with_dates(self, calendar_category, mock_client):
        """Test stock splits calendar with date parameters"""
        mock_response = [{"symbol": "EYEN", "numerator": 1, "denominator": 80}]
        mock_client._make_request.return_value = mock_response
        
        result = await calendar_category.stock_splits_calendar("2025-01-01", "2025-06-30")
