...
class TestCryptoCategory:
    ...
    @pytest.mark.asyncio
    async def test_cryptocurrency_list_response_structure(self, crypto_category, mock_client):
    ...
    async def test_quote_short_basic(self, crypto_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("quote-short", {"symbol": "BTCUSD"})
    ...
    async def test_quote_short_response_structure(self, crypto_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with("quote-short", {"symbol": "ADAUSD"})
    ...
    async def test_batch_quotes_default(self, crypto_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("batch-crypto-quotes", {"short": True})
    
    @pytest.mark.asyncio
    async def test_batch_quotes_with_short_false(self, crypto_category, mock_client):
        """Test batch quotes with short=False"""
        mock_response = [{"symbol": "BTCUSD", "price": 118741.16, "change": -37.93, "volume": 75302985728}]
        mock_client._make_request.return_value = mock_response
        
        result = await crypto_category.batch_quotes(short=False)
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("batch-crypto-quotes", {"short": False})
    ...
    async def test_historical_price_light_basic(self, crypto_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-price-eod/light", {"symbol": "BTCUSD"})
    
    @pytest.mark.asyncio
    async def test_historical_price_light_with_dates(self, crypto_category, mock_client):
        """Test historical price light with date parameters"""
        mock_response = [{"symbol": "BTCUSD", "date": "2025-07-24", "price": 118741.16, "volume": 75302985728}]
        mock_client._make_request.return_value = mock_response
        
        result = await crypto_category.historical_price_light("BTCUSD", "2025-04-25", "2025-07-25")
    ...
    @pytest.mark.asyncio
    async def test_historical_price_light_with_from_date_only(self, crypto_category, mock_client):
        """Test historical price light with only from_date parameter"""
        mock_response = [{"symbol": "BTCUSD", "date": "2025-07-24", "price": 118741.16}]
        mock_client._make_request.return_value = mock_response
        
        result = await crypto_category.historical_price_light("BTCUSD", from_date="2025-04-25")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "historical-price-eod/light",
            {"symbol": "BTCUSD", "from": "2025-04-25"}
        )
    
    @pytest.mark.asyncio
    async def test_historical_price_light_with_to_date_only(self, crypto_category, mock_client):
        """Test historical price light with only to_date parameter"""
        mock_response = [{"symbol": "BTCUSD", "date": "2025-07-24", "price": 118741.16}]
        mock_client._make_request.return_value = mock_response
        
        result = await crypto_category.historical_price_light("BTCUSD", to_date="2025-07-25")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "historical-price-eod/light",
            {"symbol": "BTCUSD", "to": "2025-07-25"}
        )
    ...
    async def test_historical_price_full_basic(self, crypto_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-price-eod/full", {"symbol": "BTCUSD"})
    ...
    async def test_historical_price_full_with_dates(self, crypto_category, mock_client):
        ...
        mock_response = [{"symbol": "BTCUSD", "date": "2025-07-24", "open": 118779.09, "close": 118741.16}]
        mock_client._make_request.return_value = mock_response
        
        result = await crypto_category.historical_price_full("BTCUSD", "2025-04-25", "2025-07-25")
    ...
    async def test_intraday_1min_basic(self, crypto_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/1min", {"symbol": "BTCUSD"})
    
    @pytest.mark.asyncio
    async def test_intraday_1min_with_dates(self, crypto_category, mock_client):
        """Test intraday 1min with date parameters"""
        mock_response = [{"date": "2025-07-24 12:29:00", "open": 118797.96, "close": 118784.04}]
        mock_client._make_request.return_value = mock_response
        
        result = await crypto_category.intraday_1min("BTCUSD", "2025-04-25", "2025-07-25")
    ...
    async def test_intraday_5min_basic(self, crypto_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/5min", {"symbol": "BTCUSD"})
    
    @pytest.mark.asyncio
    async def test_intraday_5min_with_dates(self, crypto_category, mock_client):
        """Test intraday 5min with date parameters"""
        mock_response = [{"date": "2025-07-24 12:25:00", "open": 118988.32, "close": 118797.03}]
        mock_client._make_request.return_value = mock_response
        
        result = await crypto_category.intraday_5min("BTCUSD", "2025-04-25", "2025-07-25")
    ...
    async def test_intraday_1hour_basic(self, crypto_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/1hour", {"symbol": "BTCUSD"})
    
    @pytest.mark.asyncio
    async def test_intraday_1hour_with_dates(self, crypto_category, mock_client):
        """Test intraday 1hour with date parameters"""
        mock_response = [{"date": "2025-07-24 12:00:00", "open": 119189.36, "close": 118797.03}]
        mock_client._make_request.return_value = mock_response
        
        result = await crypto_category.intraday_1hour("BTCUSD", "2025-04-25", "2025-07-25")
    ...
    @pytest.mark.asyncio
    async def test_cryptocurrency_list_response_validation(self, crypto_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_historical_price_full_response_validation(self, crypto_category, mock_client):
