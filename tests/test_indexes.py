...
class TestIndexesCategory:
    ...
    async def test_index_quote_short_basic(self, indexes_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("quote-short", {"symbol": "^GSPC"})
    ...
    async def test_all_index_quotes_with_short(self, indexes_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("batch-index-quotes", {"short": True})
    
    @pytest.mark.asyncio
    async def test_historical_price_eod_light_basic(self, indexes_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-price-eod/light", {"symbol": "^GSPC"})
    
    @pytest.mark.asyncio
    async def test_historical_price_eod_light_with_dates(self, indexes_category, mock_client):
        ...
        result = await indexes_category.historical_price_eod_light("^GSPC", from_date, to_date)
    ...
    async def test_historical_price_eod_full_basic(self, indexes_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-price-eod/full", {"symbol": "^GSPC"})
    
    @pytest.mark.asyncio
    async def test_historical_price_eod_full_with_dates(self, indexes_category, mock_client):
        ...
        result = await indexes_category.historical_price_eod_full("^GSPC", from_date, to_date)
    ...
    async def test_intraday_1min_basic(self, indexes_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/1min", {"symbol": "^GSPC"})
    ...
    async def test_intraday_5min_basic(self, indexes_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/5min", {"symbol": "^GSPC"})
    ...
    async def test_intraday_1hour_basic(self, indexes_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("historical-chart/1hour", {"symbol": "^GSPC"})
    ...
    async def test_different_date_formats(self, indexes_category, mock_client):
        ...
        mock_client._make_request.assert_called_with(
            "historical-price-eod/full",
            {"symbol": "^GSPC", "from": "2025-01-01"}
        )
        
        # Test with only to_date
        result = await indexes_category.historical_price_eod_full("^GSPC", to_date=to_date)
        assert result == mock_response
        mock_client._make_request.assert_called_with(
            "historical-price-eod/full",
            {"symbol": "^GSPC", "to": "2025-12-31"}
        )
    ...
    @pytest.mark.asyncio
    async def test_historical_data_response_validation(self, indexes_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_intraday_data_response_validation(self, indexes_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_all_index_quotes_response_validation(self, indexes_category, mock_client):
    ...
    async def test_date_edge_cases(self, indexes_category, mock_client):
        ...
        result = await indexes_category.historical_price_eod_light("^GSPC", from_date=leap_date)
        assert result == mock_response
        mock_client._make_request.assert_called_with(
            "historical-price-eod/light",
            {"symbol": "^GSPC", "from": "2024-02-29"}
        )
        
        # Test with year boundary
        year_boundary = date(2025, 12, 31)
        result = await indexes_category.historical_price_eod_light("^GSPC", to_date=year_boundary)
        assert result == mock_response
        mock_client._make_request.assert_called_with(
            "historical-price-eod/light",
            {"symbol": "^GSPC", "to": "2025-12-31"}
        ...
        result = await indexes_category.historical_price_eod_light("^GSPC", same_date, same_date)
        assert result == mock_response
        mock_client._make_request.assert_called_with(
            "historical-price-eod/light",
