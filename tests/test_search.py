...
class TestSearchCategory:
    ...
    async def test_symbols_search_with_all_params(self, search_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with(
            "search-symbol",
            {"query": "AAPL", "limit": 20, "exchange": "NASDAQ"}
        )
    ...
    async def test_screener_with_market_cap_filters(self, search_category, mock_client):
        ...
        result = await search_category.screener(
            ...
            market_cap_lower_than=5000000000
        )
    ...
    @pytest.mark.asyncio
    async def test_screener_with_sector_and_industry(self, search_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_screener_with_exchange_and_country(self, search_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_screener_with_limit_and_share_classes(self, search_category, mock_client):
