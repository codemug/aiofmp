...
class TestStatementsCategory:
    ...
    async def test_income_statement_basic(self, statements_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with("income-statement", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_income_statement_with_optional_params(self, statements_category, mock_client):
        """Test income statement with optional parameters"""
        mock_response = [{"symbol": "AAPL", "revenue": 391035000000, "netIncome": 93736000000}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.income_statement("AAPL", limit=5, period="annual")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "income-statement",
            {"symbol": "AAPL", "limit": 5, "period": "annual"}
        )
    
    @pytest.mark.asyncio
    async def test_balance_sheet_statement_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("balance-sheet-statement", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_balance_sheet_statement_with_optional_params(self, statements_category, mock_client):
        """Test balance sheet statement with optional parameters"""
        mock_response = [{"symbol": "AAPL", "totalAssets": 364980000000, "totalLiabilities": 308030000000}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.balance_sheet_statement("AAPL", limit=3, period="quarter")
    ...
    async def test_cash_flow_statement_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("cash-flow-statement", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_cash_flow_statement_with_optional_params(self, statements_category, mock_client):
        """Test cash flow statement with optional parameters"""
        mock_response = [{"symbol": "AAPL", "operatingCashFlow": 118254000000, "freeCashFlow": 108807000000}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.cash_flow_statement("AAPL", limit=4, period="Q4")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "cash-flow-statement",
            {"symbol": "AAPL", "limit": 4, "period": "Q4"}
        )
    ...
    async def test_key_metrics_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("key-metrics", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_key_metrics_with_optional_params(self, statements_category, mock_client):
        """Test key metrics with optional parameters"""
        mock_response = [{"symbol": "AAPL", "marketCap": 3495160329570, "enterpriseValue": 3571846329570}]
        mock_client._make_request.return_value = mock_response
    ...
    @pytest.mark.asyncio
    async def test_financial_ratios_with_optional_params(self, statements_category, mock_client):
        """Test financial ratios with optional parameters"""
        mock_response = [{"symbol": "AAPL", "grossProfitMargin": 0.462, "netProfitMargin": 0.240}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.financial_ratios("AAPL", limit=1, period="Q4")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "ratios",
            {"symbol": "AAPL", "limit": 1, "period": "Q4"}
        )
    ...
    async def test_financial_scores_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("financial-scores", {"symbol": "AAPL"})
    ...
    async def test_owner_earnings_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("owner-earnings", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_owner_earnings_with_limit(self, statements_category, mock_client):
        """Test owner earnings with limit parameter"""
        mock_response = [{"symbol": "AAPL", "ownersEarnings": 27655035250, "ownersEarningsPerShare": 1.83}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.owner_earnings("AAPL", limit=3)
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("owner-earnings", {"symbol": "AAPL", "limit": 3})
    ...
    async def test_enterprise_values_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("enterprise-values", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_enterprise_values_with_optional_params(self, statements_category, mock_client):
        """Test enterprise values with optional parameters"""
        mock_response = [{"symbol": "AAPL", "enterpriseValue": 3571846329570}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.enterprise_values("AAPL", limit=2, period="quarter")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "enterprise-values",
            {"symbol": "AAPL", "limit": 2, "period": "quarter"}
        )
    
    @pytest.mark.asyncio
    async def test_income_statement_growth_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("income-statement-growth", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_income_statement_growth_with_optional_params(self, statements_category, mock_client):
        """Test income statement growth with optional parameters"""
        mock_response = [{"symbol": "AAPL", "growthRevenue": 0.0202, "growthNetIncome": -0.0336}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.income_statement_growth("AAPL", limit=4, period="annual")
    ...
    @pytest.mark.asyncio
    async def test_balance_sheet_statement_growth_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("balance-sheet-statement-growth", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_balance_sheet_statement_growth_with_optional_params(self, statements_category, mock_client):
        """Test balance sheet statement growth with optional parameters"""
        mock_response = [{"symbol": "AAPL", "growthTotalAssets": 0.0352, "growthTotalEquity": -0.0836}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.balance_sheet_statement_growth("AAPL", limit=3, period="Q4")
    ...
    @pytest.mark.asyncio
    async def test_cash_flow_statement_growth_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("cash-flow-statement-growth", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_cash_flow_statement_growth_with_optional_params(self, statements_category, mock_client):
        """Test cash flow statement growth with optional parameters"""
        mock_response = [{"symbol": "AAPL", "growthOperatingCashFlow": 0.0698, "growthFreeCashFlow": 0.0926}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.cash_flow_statement_growth("AAPL", limit=2, period="annual")
    ...
    @pytest.mark.asyncio
    async def test_financial_statement_growth_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("financial-growth", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_financial_statement_growth_with_optional_params(self, statements_category, mock_client):
        """Test financial statement growth with optional parameters"""
        mock_response = [{"symbol": "AAPL", "revenueGrowth": 0.0202, "netIncomeGrowth": -0.0336}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.financial_statement_growth("AAPL", limit=5, period="quarter")
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "financial-growth",
            {"symbol": "AAPL", "limit": 5, "period": "quarter"}
        )
    
    @pytest.mark.asyncio
    async def test_revenue_product_segmentation_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("revenue-product-segmentation", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_revenue_product_segmentation_with_optional_params(self, statements_category, mock_client):
        """Test revenue product segmentation with optional parameters"""
        mock_response = [{"symbol": "AAPL", "data": {"iPhone": 201183000000, "Mac": 29984000000}}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.revenue_product_segmentation("AAPL", period="quarter", structure="flat")
    ...
    @pytest.mark.asyncio
    async def test_revenue_geographic_segmentation_basic(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_once_with("revenue-geographic-segmentation", {"symbol": "AAPL"})
    
    @pytest.mark.asyncio
    async def test_revenue_geographic_segmentation_with_optional_params(self, statements_category, mock_client):
        """Test revenue geographic segmentation with optional parameters"""
        mock_response = [{"symbol": "AAPL", "data": {"Americas": 167045000000, "Europe": 101328000000}}]
        mock_client._make_request.return_value = mock_response
        
        result = await statements_category.revenue_geographic_segmentation("AAPL", period="annual", structure="flat")
    ...
    async def test_large_response_handling(self, statements_category, mock_client):
        ...
        mock_client._make_request.assert_called_once_with("income-statement", {"symbol": "AAPL", "limit": 20})
    
    @pytest.mark.asyncio
    async def test_response_structure_validation(self, statements_category, mock_client):
    ...
    async def test_parameter_combinations(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_with("income-statement", {"symbol": "AAPL", "limit": 5})
        
        # Test with only period
        result = await statements_category.income_statement("AAPL", period="annual")
        assert result == mock_response
        mock_client._make_request.assert_called_with("income-statement", {"symbol": "AAPL", "period": "annual"})
        
        # Test with both parameters
        result = await statements_category.income_statement("AAPL", limit=5, period="annual")
        assert result == mock_response
        mock_client._make_request.assert_called_with("income-statement", {"symbol": "AAPL", "limit": 5, "period": "annual"})
    ...
    async def test_different_symbols(self, statements_category, mock_client):
        ...
        assert result == mock_response
        mock_client._make_request.assert_called_with("income-statement", {"symbol": "MSFT"})
        
        # Test with Google
        result = await statements_category.income_statement("GOOGL")
        assert result == mock_response
        mock_client._make_request.assert_called_with("income-statement", {"symbol": "GOOGL"})
        
        # Test with Tesla
        result = await statements_category.income_statement("TSLA")
        assert result == mock_response
        mock_client._make_request.assert_called_with("income-statement", {"symbol": "TSLA"})
    
    @pytest.mark.asyncio
    async def test_income_statement_response_validation(self, statements_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_balance_sheet_response_validation(self, statements_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_cash_flow_response_validation(self, statements_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_scores_response_validation(self, statements_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_revenue_segmentation_response_validation(self, statements_category, mock_client):
