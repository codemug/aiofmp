...
class TestConfigFallback:
    def test_uses_current_directory_default(
        self, env_with_key, runner, tmp_path, monkeypatch
    ) -> None:
    ...
    def test_errors_when_no_config_found(
        self, env_with_key, runner, tmp_path, monkeypatch
    ) -> None:
        monkeypatch.chdir(tmp_path)
        ...
        assert result.exit_code != 0
        assert (
            "harvester.yaml" in result.output.lower()
