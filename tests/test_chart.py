...
class TestChartCategory:
    ...
    async def test_historical_price_light_with_dates(self, chart_category, mock_client):
        ...
        result = await chart_category.historical_price_light("AAPL", "2025-01-01", "2025-03-31")
    ...
    async def test_historical_price_full_with_dates(self, chart_category, mock_client):
        ...
        result = await chart_category.historical_price_full("AAPL", "2025-01-01", "2025-03-31")
    ...
    @pytest.mark.asyncio
    async def test_historical_price_unadjusted_with_dates(self, chart_category, mock_client):
        """Test historical price unadjusted with date parameters"""
        mock_response = [{"symbol": "AAPL", "date": "2025-02-04", "adjClose": 232.8}]
        mock_client._make_request.return_value = mock_response
        
        result = await chart_category.historical_price_unadjusted("AAPL", "2025-01-01", "2025-03-31")
    ...
    @pytest.mark.asyncio
    async def test_historical_price_dividend_adjusted_basic(self, chart_category, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_historical_price_dividend_adjusted_with_dates(self, chart_category, mock_client):
        """Test historical price dividend adjusted with date parameters"""
        mock_response = [{"symbol": "AAPL", "date": "2025-02-04", "adjClose": 232.8}]
        mock_client._make_request.return_value = mock_response
        
        result = await chart_category.historical_price_dividend_adjusted("AAPL", "2025-01-01", "2025-03-31")
    ...
    async def test_intraday_1min_with_all_params(self, chart_category, mock_client):
        ...
        result = await chart_category.intraday_1min("AAPL", "2025-01-01", "2025-01-02", False)
        
        assert result == mock_response
        mock_client._make_request.assert_called_once_with(
            "historical-chart/1min",
            {"symbol": "AAPL", "from": "2025-01-01", "to": "2025-01-02", "nonadjusted": False}
        )
    ...
    async def test_large_response_handling(self, chart_category, mock_client):
        ...
        result = await chart_category.historical_price_light("AAPL", "2025-01-01", "2025-04-10")
    ...
    async def test_date_parameter_combinations(self, chart_category, mock_client):
        ...
        result = await chart_category.historical_price_light("AAPL", to_date="2025-03-31")
        assert result == mock_response
        mock_client._make_request.assert_called_with(
            "historical-price-eod/light",
    ...
    @pytest.mark.asyncio
    async def test_nonadjusted_parameter_combinations(self, chart_category, mock_client):
